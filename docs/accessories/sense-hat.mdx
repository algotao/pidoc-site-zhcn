...
Sense HAT 和 Sense HAT V2 的原理图和机械图纸 可从[产品信息门户](https://pip.raspberrypi.com/categories/676-raspberry-pi-sense-hat)下载。
...
### 操纵杆
操纵杆作为输入事件设备出现，名为 `Raspberry Pi Sense HAT Joystick`，映射到方向键和 **回车键**。任何能够处理输入的库都应支持它，或者直接通过 [evdev 接口](https://www.kernel.org/doc/Documentation/input/input.txt) 支持。合适的库包括 [SDL](https://www.libsdl.org/)、[pygame](https://www.pygame.org/docs/) 和 [python-evdev](https://python-evdev.readthedocs.org/en/latest/) 等。附带的 `snake` 示例展示了如何直接访问操纵杆。
