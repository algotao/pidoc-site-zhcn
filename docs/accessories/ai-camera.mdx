...
sidebar_position: 8
draft: false
description: Raspberry Pi AI相机搭载索尼IMX500成像传感器，为各类相机应用提供低延迟、高性能的AI功能。本文详细介绍了如何在Raspberry Pi 5或4 Model B上安装和配置AI相机，包括固件安装、系统更新及运行预装神经网络模型（如MobileNet SSD和PoseNet）的步骤。通过rpicam-apps和Picamera2等工具，用户可以轻松部署和运行自定义神经网络模型，实现物体检测、姿态估计等功能。文章还深入解析了AI相机的工作原理、系统架构及模型部署流程，帮助开发者充分利用AI加速器，提升图像处理效率。
keywords: [Raspberry Pi AI相机, 索尼IMX500, 神经网络模型, rpicam-apps, Picamera2, MobileNet SSD, PoseNet, 物体检测, 姿态估计, AI加速器, 图像处理, 模型部署, 输入张量, 输出张量, Raspberry Pi 5, Raspberry Pi 4 Model B]
last_update: 
  date: 2025-12-20
---
...
<img src="/img/accessories/ai-camera/ai-camera.jpg" alt="树莓派人工智能相机" width="80%" />
*树莓派人工智能相机*
...
该网络的原始推理输出数据可能存在较多噪声，因此本阶段还会执行时间滤波并应用迟滞效应。若需禁用此滤波功能，请移除`temporal_filter`配置块。
...
1. 提供浮点神经网络模型（PyTorch 或 TensorFlow）。
2. 通过 Edge-MDT（边缘人工智能模型开发工具包）运行该模型。
    * a. 对模型进行**量化**与压缩，使其能够利用 IMX500 相机模块的可用资源运行。
    * b. 将压缩后的模型**转换**为 IMX500 格式。
3. 将模型打包成固件文件，以便在运行时加载到相机上。
...
模型通过索尼模型压缩工具包（MCT）进行量化和压缩。该工具会在安装Edge-MDT时自动安装。更多信息请参阅[索尼模型优化GitHub仓库](https://github.com/sony/model_optimization)。
...
:::important[重要]
为充分利用IMX500传感器加速器可用的内存，请在上述命令中添加`--no-input-persistency`参数。但此操作将禁用输入张量生成功能，该功能可能用于调试目的。
:::
