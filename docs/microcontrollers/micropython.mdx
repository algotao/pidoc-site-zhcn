---
sidebar_position: 5
draft: false
description: MicroPython 是 Python 3 的精简高效实现，专为像树莓派 Pico 这样的嵌入式硬件设计，支持通过 USB 进行交互式编程（REPL），并内置文件系统。本文介绍了如何通过简单的拖放 UF2 文件方式在树莓派 Pico 或 Pico W 上安装 MicroPython，以及如何利用其访问底层硬件、连接 Wi-Fi 和蓝牙等功能。同时提供了丰富的官方文档链接与资源，帮助开发者快速上手，适用于 RP2040 和 RP2350 等芯片平台。
keywords: [MicroPython, 树莓派 Pico, 树莓派 Pico W, RP2040, 嵌入式编程, UF2 文件, REPL, 交互式编程, Wi-Fi, 蓝牙, Python, 硬件编程, Thonny IDE, 树莓派官方文档, MicroPython 固件, sys模块, network模块, 嵌入式硬件, C/C++]
last_update: 
  date: 2026-08-02
---
import Normal from "../_normal.mdx";

# MicroPython

## 什么是MicroPython？
MicroPython 是 Python 3 编程语言的完整实现，可直接在树莓派Pico等嵌入式硬件上运行。您可以通过 USB 串行接口获得交互式提示（REPL）来立即执行命令，并获得一个内置文件系统。MicroPython 的 Pico 端口包括用于访问低级芯片特定硬件的模块。
- [MicroPython 维基](https://github.com/micropython/micropython/wiki)
- [MicroPython 论坛](https://forum.micropython.org/)

## 拖放MicroPython
您可以通过 USB 将 Pico 连接到电脑，然后将文件拖放到 Pico 上对其进行编程，因此我们提供了一个可下载的 UF2 文件，让您可以更轻松地安装 MicroPython。

<video controls src="/img/microcontrollers/micropython/MicroPython.webm" alt="MicroPython" width="80%" />

为你的电路板下载正确的 MicroPython UF2 文件：

* [Pico](https://micropython.org/download/rp2-pico/rp2-pico-latest.uf2)
* [Pico W](https://micropython.org/download/rp2-pico-w/rp2-pico-w-latest.uf2)
* [Pico 2](https://micropython.org/download/RPI_PICO2/RPI_PICO2-latest.uf2)
* [Pico 2 W](https://micropython.org/download/RPI_PICO2_W/RPI_PICO2_W-latest.uf2)

有关使用 C/C++ 或 MicroPython 在 Raspberry Pi Pico W 系列设备上使用 Wi-Fi 和蓝牙的更多信息，请参阅 [Pico W 系列设备的 Wi-Fi 和蓝牙连接](https://pip.raspberrypi.com/documents/RP-008257-DS)。

有关 [支持的蓝牙协议和配置文件](https://github.com/bluekitchen/btstack#supported-protocols-and-profiles) 的更多信息，请参阅 Blue Kitchen [BTStack](https://github.com/bluekitchen/btstack) Github 代码库。

:::tip
[MicroPython 下载页面](https://micropython.org/download/) 上有其他基于 RP2040 和 RP2350 的板的 MicroPython 分发版。
:::

要对设备进行编程，请按照以下步骤操作：

1. 用 USB 电缆将 Pico 与电脑连接时，按住 BOOTSEL 按钮。当 Pico 显示为名为 RPI-RP2（Pico）或 RP2350（Pico 2）的大容量存储设备时，松开 BOOTSEL 按钮。
2. 将 MicroPython UF2 文件拖放到已挂载的存储卷上。Pico 将重新启动。现在您正在运行 MicroPython。
3. 通过 USB 串口访问 REPL。

[《树莓派 Pico 系列 Python SDK》](https://datasheets.raspberrypi.com/pico/raspberry-pi-pico-python-sdk.pdf) 一书包含连接 Pico 并使用命令行和 [Thonny](https://thonny.org/) IDE 在 MicroPython 中编程的分步说明。

## 在哪里可以找到文档？
有关 Raspberry Pi 微控制器的 MicroPython 移植信息，请访问；

[树莓派Pico Python SDK](https://pip.raspberrypi.com/documents/RP-008355-DS)<br />
&emsp;&emsp;用于 Raspberry Pi 微控制器的 MicroPython 环境

[Pico W 系列设备的 Wi-Fi 和蓝牙连接](https://pip.raspberrypi.com/documents/RP-008257-DS)<br />
&emsp;&emsp;在 Pico W 系列设备上使用 C/C++ 或 MicroPython 进行无线连接

[RP2快速参考](https://docs.micropython.org/en/latest/rp2/quickref.html)<br />
&emsp;&emsp;有关 MicroPython RP2XXX 移植的官方文档

[RP2库](https://docs.micropython.org/en/latest/library/rp2.html)<br />
&emsp;&emsp;有关 MicroPython 中 rp2 模块的官方文档

[pico-micropython-examples 代码库](https://github.com/raspberrypi/pico-micropython-examples)<br />
&emsp;&emsp;MicroPython 示例项目

## 我在哪个硬件上运行？

对于用 MicroPython 编写的软件，没有直接的方法可以通过查看硬件来确定它是运行在树莓派 Pico 还是 Pico W 上。不过，您可以通过查看特定 MicroPython 固件中是否包含网络功能来间接判断：

```py
import network
if hasattr(network, "WLAN"):
   # 电路板具有 WLAN 功能
```
另外，您也可以使用 `sys` 模块检查 MicroPython 固件版本，以查看它是针对树莓派 Pico 还是 Pico W编译的。
```py
>>> import sys
>>> sys.implementation
(name='micropython', version=(1, 19, 1), _machine='Raspberry Pi Pico W with RP2040', _mpy=4102)
```

如果 `Pico W` 字符串出现在 `sys.implement._machine` 中，则固件是为 Pico W 编译的。

<Normal />
