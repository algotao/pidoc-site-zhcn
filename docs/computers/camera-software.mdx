...
如果您的 Raspberry Pi 有两个摄像头连接器（例如 Raspberry Pi 5 或其中一个计算模块），那么您可以通过在上表中使用的 `dtoverlay` 中添加 `,cam0` 或 `,cam1`（不要添加任何空格）来指定您所指的是哪一个。如果不添加其中任何一个，它将默认检查摄像头连接器 1 (`cam1`)。但请注意，对于正式的 Raspberry Pi 摄像头模块，自动检测将正确识别连接到设备的所有摄像头。
