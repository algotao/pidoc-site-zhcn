...
sidebar_position: 8
draft: false
title_meta: 在树莓派运行 libcamera 及 rpicam 等系列工具实现拍照、延时摄影、记录视频(官网24年12月)
...
本节介绍从 `rpicam-vid` 进行本机流式传输。您也可以使用 [`libav`](#libav-选项) 后端进行网络流式传输。
...
要在服务器上抑制预览窗口，请使用 [`nopreview`](#nopreview)。

使用 [`inline`](#inline) 标志可强制在每帧内包含流标题信息，这有助于客户端在错过开头时理解流。

### libav

您可以将 `libav` 后端用作音频/视频的网络流媒体源。
要使用 Raspberry Pi 作为服务器通过 TCP 串流视频，请使用以下命令，用客户端的 IP 地址或组播地址替换 `<ip-addr>` 占位符，用您希望用于串流的端口替换 `<port>` 占位符：
...
[GStreamer](https://gstreamer.freedesktop.org/) 是一个用于读取、处理和播放多媒体文件的 Linux 框架。本节介绍如何使用 `rpicam-vid` 通过网络串流视频。
