...
### 桌面
您可通过控制中心中的**屏幕空白**选项控制屏幕空白功能。

#### 控制中心
单击菜单栏中的树莓派按钮。导航至 **首选项** > **控制中心**。

:::tip
在树莓派操作系统早期版本中，控制中心应用程序名为树莓派配置。
:::

<img src="/img/computers/configuration/pi-configuration.png" alt="从桌面打开 控制中心 菜单" />

选择 **显示** 选项卡。将 **屏幕休眠** 单选按钮切换到开启位置。按 **确定** 确认选择。

<img src="/img/computers/configuration/blanking.png" alt="在控制中心菜单中切换屏幕休眠" />
...
### 控制台
`dpms_timeout`屏幕休眠配置仅影响桌面会话。在**控制台模式**下，当树莓派仅通过终端连接显示器和键盘时，请在内核命令行中使用`consoleblank`设置。
