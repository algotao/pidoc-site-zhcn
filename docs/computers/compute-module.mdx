...
要将显示器连接到 DISP1/DSI1：
1. 断开计算模块的电源。
2. 通过22W至15W显示适配器，将显示器连接至计算模块I/O板上的DISP1/DSI1端口。
3. (仅限 CM1、CM3、CM3+ 和 CM4S）： 用跳线连接以下 GPIO 引脚：
  - `0` 至 `CD1_SDA`
  - `1` 至 `CD1_SCL`
4. _（CM5）_ 在计算模块 5 的 I/O 板上，按照丝印标记在 J6 上添加相应的跳线。
5. 重新连接计算模块的电源。
6. 在 [/boot/firmware/config.txt](./config-txt.mdx#什么是configtxt) 中添加以下一行：
```ini
dtoverlay=vc4-kms-dsi-7inch
```
7. 使用 `sudo reboot` 重启计算模块。设备应能检测到并开始向显示器显示输出。

### 将显示器连接到DISP0/DSI0

要将显示器连接到 DISP0/DSI0：
1. 通过22W至15W显示适配器，将显示器连接至计算模块I/O板上的DISP0/DSI0端口。
2. (仅限 CM1、CM3、CM3+ 和 CM4S）： 用跳线连接以下 GPIO 引脚：
  - `28` 至 `CD0_SDA`
  - `29` 至 `CD0_SCL`
3. _（仅限CM4）_ 在计算模块4的I/O板上，按照丝印标记在J6上添加相应的跳线。
4. 重新连接计算模块的电源。
5. 在 `/boot/firmware/config.txt `中添加以下一行：
```ini
dtoverlay=vc4-kms-dsi-7inch
```
6. 使用 `sudo reboot` 重启计算模块。设备应能检测到并开始向显示屏显示输出。
...
Touch Display 2 是专为 Raspberry Pi 设备设计的 720x1280 7" LCD 显示屏（见 https://www.raspberrypi.com/products/touch-display-2/ ）。它的连接方式与原始触摸显示器相同，但计算模块上的软件设置略有不同，因为它使用不同的显示驱动程序。有关连接详情，请参阅 [触摸显示屏2](../accessories/touch-display-2.mdx)。

编辑 /boot/firmware/config.txt 文件并添加以下内容，以便在 DISP1/DSI1 上启用触摸显示屏 2。您还需要在 J6 上添加丝印所示的跳线。
