...
sidebar_position: 11
draft: false
title_meta: 在树莓派计算模块(CM)进行烧录、连接与启用外设
...
**Lite** 版本的计算模块没有板载 eMMC。相反，请按照 [安装操作系统](./getting-started.mdx#将操作系统安装到启动介质上) 中为其他 Raspberry Pi 设备烧录存储设备的步骤进行操作。
...
1.  下载 [releases](https://github.com/raspberrypi/usbboot/releases) 或 [从源码编译 `rpiboot`](https://github.com/raspberrypi/usbboot)。
2. 双击安装程序运行。这将安装驱动程序和启动工具。请勿关闭安装过程中出现的任何驱动程序安装窗口。
3. 重新启动
4. 连接 IO 板电源。Windows 将发现硬件并配置所需的驱动程序。
5. 从开始菜单中选择 **rpiboot - 大容量存储设备**。几秒钟后，计算模块的 eMMC、NVMe 或 SD 卡将作为 USB 大容量存储设备显示。这同时还会提供一个作为串行端口设备的调试控制台。
...
你可以使用 [Raspberry Pi Imager](./getting-started.mdx#使用-imager-安装) 将操作系统映像烧录到计算模块。
...
## 计算模块 EEPROM bootloader {#计算模块-eeprom-bootloader}
...
[`minimal-cm-dt-blob.dts`](https://pip.raspberrypi.com/documents/RP-008166-DS)是一个 `.dts` 设备树文件示例。它设置了 HDMI 热插拔检测和 ACT LED，并将所有其他 GPIO 设置为默认输入。
...
下载 [`minimal-cm-dt-blob.dts`](https://pip.raspberrypi.com/documents/RP-008166-DS)，并将其复制到 `/boot/firmware/` 中的启动分区。
...
下载 [`example1-overlay.dts`](https://pip.raspberrypi.com/documents/RP-008167-DS)，将其复制到 `/boot/firmware/` 中的启动分区，然后使用以下命令编译：
...
配置摄像头前，请[确保您的系统运行最新的可用软件](./raspberry-pi.mdx#更新-bootloader)：
...
* [CM5数据表](https://pip.raspberrypi.com/documents/RP-008180-DS)
...
* [CM5IO 数据表](https://pip.raspberrypi.com/documents/RP-008182-DS)
...
* [CM4 数据表］(https://pip.raspberrypi.com/documents/RP-008168-DS)

:::note
白皮书[配置计算模块 4](https://pip.raspberrypi.com/documents/RP-003470-WP)
...
* [CM4IO 数据表](https://pip.raspberrypi.com/documents/RP-008172-DS)

我们还提供 CM4 IO 板的 KiCad PCB 设计集：

* [CM4IO KiCad 文件](https://pip.raspberrypi.com/documents/RP-008173-DS)

### 计算模块 4S 数据表

计算模块 4S (CM4S) 以 CM1、CM3 和 CM3+ 的 DDR2-SODIMM 外形提供 CM4 的内部结构。要了解有关 CM4S 的更多信息，请参阅以下文档：

* [CM4S 数据表](https://pip.raspberrypi.com/documents/RP-008177-DS)

### 计算模块 3+ 数据表

计算模块 3+ (CM3+) 是一种受支持的产品，其生命周期结束 (EOL) 日期不早于 2028 年 1 月。要了解有关 CM3+ 及其相应 IO 板的更多信息，请参阅以下文档：

* [CM3+数据表](https://pip.raspberrypi.com/documents/RP-008163-DS)

### 计算模块 1 和计算模块 3 数据表

Raspberry Pi 计算模块 1 (CM1) 和计算模块 3 (CM3) 是受支持的产品，其生命周期终止 (EOL) 日期不早于 2026 年 1 月。要了解有关 CM1 和 CM3 的更多信息，请参阅以下文档：

* [CM1和CM3数据表](https://pip.raspberrypi.com/documents/RP-008161-DS)。
* [CM1原理图](https://pip.raspberrypi.com/documents/RP-008162-DS)。
* [CM3原理图](https://pip.raspberrypi.com/documents/RP-008164-DS)。
...
* [CMIO原理图](https://pip.raspberrypi.com/documents/RP-008256-DS)
* [CMIO1.2版（CMIO/CMIO1）设计文件](https://pip.raspberrypi.com/documents/RP-008376-DS)
* [CMIO3.0版（CMIO3）设计文件](https://pip.raspberrypi.com/documents/RP-008377-DS)

### 计算模块摄像头/显示器适配器板原理图

计算模块摄像头/显示器适配器板（CMCDA）为计算模块提供摄像头和显示器接口。要了解有关 CMCDA 的更多信息，请参阅以下文档：

* [CMCDA原理图](https://pip.raspberrypi.com/documents/RP-008255-DS)
* [CMCDA1.1版设计文件](https://pip.raspberrypi.com/documents/RP-008375-DS)
