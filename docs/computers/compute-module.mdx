...
* [CM5IO外壳](#cm5io-外壳)，一款为CM5IO（及连接的CM5）设计的两件式金属外壳。该外壳还可选配天线和散热器。
* [天线（CM4和CM5）](#天线cm4和cm5)，支持2.4 GHz和5 GHz频段的天线，通过CM4或CM5实现无线连接。
* [CM5散热器](#cm5-散热器)，用于散发CM5热量的被动式散热片。
...
计算模块5 I/O板外壳为CM5IO及其连接的CM5提供物理保护，包含：
...
CM5IO机箱采用双件式结构，组装后尺寸为170毫米×94毫米×28毫米。机箱由金属板材制成，重量约350克。为实现热管理，机箱内置风扇可将气流导向CM5和CM5IO组件。您可根据散热需求拆卸或更换风扇。您还可选配 [CM5散热器](#cm5-散热器) 以提升散热性能。
...
1. **可选：安装散热器** 如需安装散热器，请参照[CM5散热器安装指南](#cm5-散热器)操作。若同时安装天线，请先连接天线的U.FL接口以方便后续操作。
...
该天线支持双频Wi-Fi，可连接至CM4或CM5主板的[U.FL连接器](https://en.wikipedia.org/wiki/Hirose_U.FL)。天线完全展开时高度为108毫米；SMA转U.FL连接线长度为205毫米。
...
CM5散热器的尺寸为41毫米×56毫米×12.7毫米。该散热器采用铝制散热片结构，底部配有导热硅胶垫。新版 [CM5IO外壳](#cm5io-外壳) 支持在机箱内部同时使用散热器和预装风扇。若您使用的是旧版CM5IO外壳，必须拆卸外壳内的风扇以腾出散热器安装空间。
