...
sidebar_position: 9
draft: false
title_meta: 树莓派1 至 树莓派5 硬件的完整介绍，最详细的树莓派资料
description: 全面了解树莓派系列硬件的多样性及其应用。本文详细介绍树莓派旗舰系列、键盘系列、Zero系列、Compute Module系列和Pico微控制器系列的规格与特点，包括处理器、内存、GPIO接口、无线连接、电源管理等。此外，还探讨了散热方案、电源需求、USB-C电源、以太网供电(PoE)等功能，为树莓派用户提供全面的硬件知识。
keywords: [树莓派硬件, 旗舰系列, 键盘系列, Zero系列, Compute Module系列, Pico微控制器, 处理器, 内存, GPIO接口, 无线连接, 电源管理, 散热方案, USB-C电源, 以太网供电]
last_update: 
  date: 2026-08-02
---
...
* **[单板计算机(SBCs)](./raspberry-pi.mdx#旗舰级单板计算机sbc系列)**。旗舰级的树莓派系列在信用卡大小的机身内，集成了高性能硬件、完整的Linux操作系统以及多种通用接口。
* **[键盘计算机(X00)](./raspberry-pi.mdx#键盘计算机系列x00)**。树莓派键盘计算机将单板计算机硬件、完整的Linux操作系统及常用接口整合于紧凑的键盘外壳内。
* **[Zero](./raspberry-pi.mdx#zero系列)**。采用极简设计的小型经济型机型，提供完整的Linux操作系统和基本接口，具有低功耗特性。
* **[计算模块(CM)](./raspberry-pi.mdx#compute-module系列)**。 专为嵌入式及工业应用设计的板卡，提供与旗舰级单板计算机相同的硬件配置，但采用更小尺寸且无板载接口。需连接独立底板以获取必要端口和引脚。
* **[Pico(微控制器)](./raspberry-pi.mdx#pico微控制器)**。小巧多功能板卡，不运行Linux系统。程序直接刷写至板载存储器，无需可移除存储介质。Pico微控制器适用于实时控制与轻量级嵌入式项目。
...
:::important[重要]
由于核心SoC停产，树莓派2B的1.1版和1.2版已达到产品生命周期终止（EoL）。官方EoL日期为2025年10月16日。树莓派2B修订版1.3采用相同机械尺寸并搭载BCM2837B0处理器，因此推荐用于现有设计。更多详情请参阅官方[停产通知](https://pip.raspberrypi.com/documents/RP-009284-PC)。
:::
...
<table>
    ...
    <tr>
        ...
        <td>
          - revision 1.1 [BCM2836](./processors.mdx#bcm2836)
          - revision 1.2 [BCM2837](./processors.mdx#bcm2837)
          - revision 1.3 [BCM2837b0](./processors.mdx#bcm2837b0)
        </td>
        <td>1 GB</td>
        <td>40针GPIO头</td>
        <td>
    ...
    <tr>
        ...
        <td>
          - revision 1.1[BCM2837](./processors.mdx#bcm2837)
          - revision 1.2[BCM2837B0](./processors.mdx#bcm2837b0)
        </td>
        <td>1 GB</td>
        <td>40针GPIO头</td>
        <td>
    ...
    <tr>
        ...
        <td>1 GB<br/>2 GB<br/>3 GB<br/>4 GB<br/>8 GB</td>
        <td>40针GPIO头</td>
        <td>
          - 2× 微型 HDMI
          - 2× USB 2.0
    ...
    <tr>
        ...
        <td>1 GB<br/>2 GB<br/>4 GB<br/>8 GB<br/>16 GB</td>
        <td>40针GPIO头</td>
        <td>
          - 2× 微型 HDMI
          - 2× USB 2.0
          ...
          - 单通道 [PCIe FFC 连接器](https://pip.raspberrypi.com/documents/RP-008298-DS)
          - UART 连接器
          - RTC 电池连接器
...
键盘系列设备采用`<X00>`形式的型号标识符，其中`X`代表对应的旗舰单板计算机（SBC）系列设备。例如，树莓派400即为树莓派4的键盘版本。有关树莓派键盘计算机的详细信息，请参阅[键盘计算机](./keyboard-computers.mdx)。
...
<table>
    ...
    <tr>
     ...
     <tr>
        <td align="center"><img src="/img/computers/raspberry-pi/500plus.png" alt="Raspberry Pi 500+"/>*Raspberry Pi 500+*</td>
        <td>[BCM2712](./processors.mdx#bcm2712)</td>
        <td>16 GB</td>
        <td>40针GPIO头</td>
        <td>
...
:::important[重要]
由于核心SoC停产，树莓派计算模块3（CM3）和计算模块3精简版（CM3Lite）已达到产品生命周期终止（EoL）。官方EoL日期为2025年10月16日。与CM3最接近的替代品是树莓派计算模块3+，其提供相同的机械尺寸、改进的散热设计以及BCM2837B0处理器，因此推荐用于现有设计。对于需要SODIMM规格的新设计，建议选用计算模块4S；其他新设计则推荐计算模块4或计算模块5。更多详情请参阅官方[停产通知](https://pip.raspberrypi.com/documents/RP-009286-PC)。
:::

有关树莓派计算模块硬件的更多详细信息，请参阅 [计算模块硬件](./compute-module.mdx)。
...
<table>
    ...
    <tr>
        ...
        <td>[BCM2712](./processors.mdx#bcm2712)</td>
        <td>2GB<br />4GB<br />8GB<br />16GB</td>
    ...
    <tr>
        <td align="center"><img src="/img/computers/raspberry-pi/compute-module-0.jpg" alt="Raspberry Pi Compute Module 0"/>*Raspberry Pi Compute Module 0*</td>
        <td>[RP3A0](./processors.mdx#rp3a0)</td>
        <td>512MB</td>
        <td>0GB (精简版)<br />8GB<br />16GB</td>
        <td>132引脚 39*33mm封装</td>
        <td>
          可选：
          - 2.4GHz 单频 802.11n Wi-Fi (35Mb/s)
          - 蓝牙 4.2、低功耗蓝牙 (BLE)
...
:::tip
使用 DDR2 SO-DIMM 物理外形尺寸的计算模块与 DDR2 SODIMM 电气规格**不兼容**。
:::
...
<table>
    ...
    <tr>
        ...
        <td>[RP2040](../microcontrollers/microcontroller-chips#rp2040)</td>
        <td>264kB</td>
        <td>2MB</td>
    ...
    <tr>
        ...
        <td>[RP2350](../microcontrollers/microcontroller-chips#rp2350)</td>
        <td>520kB</td>
        <td>4MB</td>
        <td>2个20针GPIO头(未填充)</td>
        <td>无</td>
    ...
    <tr>
        <td align="center"><img src="/img/computers/raspberry-pi/pico-2-h.jpg" alt="Raspberry Pi Pico 2 with headers"/>*Raspberry Pi Pico 2 with headers*</td>
        <td>[RP2350](../microcontrollers/microcontroller-chips#rp2350)</td>
        <td>520kB</td>
        <td>4MB</td>
        <td>2个20针GPIO头(未填充)</td>
    ...
    <tr>
        <td align="center"><img src="/img/computers/raspberry-pi/pico-2-wh.jpg" alt="Raspberry Pi Pico 2 W with headers"/>*Raspberry Pi Pico 2 W with headers*</td>
        <td>[RP2350](../microcontrollers/microcontroller-chips#rp2350)</td>
        <td>520kB</td>
        <td>4MB</td>
        <td>2个20针GPIO头(未填充)</td>
...
### 树莓派5
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008347-DS)
- 树莓派5的[STEP文件](https://pip.raspberrypi.com/documents/RP-008369-DS)

### 树莓派4B
- [原理图，修订版4.0](https://pip.raspberrypi.com/documents/RP-008345-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008343-DS)
- [机械制图，DXF](https://pip.raspberrypi.com/documents/RP-008342-DS)

### 树莓派3B+
- [原理图，修订版 1.0](https://pip.raspberrypi.com/documents/RP-008339-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008337-DS)
- [机械制图，DXF](https://pip.raspberrypi.com/documents/RP-008336-DS)
- [案例图纸，PDF](https://pip.raspberrypi.com/documents/RP-008160-DS)

### 树莓派3A+
- [原理图，修订版 1.0](https://pip.raspberrypi.com/documents/RP-008332-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008330-DS)
- [案例图纸，PDF](https://pip.raspberrypi.com/documents/RP-008329-DS)

### 树莓派3B
- [原理图，修订版 1.2](https://pip.raspberrypi.com/documents/RP-008340-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008335-DS)
- [机械制图，DXF](https://pip.raspberrypi.com/documents/RP-008334-DS)

### 树莓派2B
- [原理图，修订版 1.2](https://pip.raspberrypi.com/documents/RP-008327-DS)

### 树莓派1B+
- [原理图，修订版 1.2](https://pip.raspberrypi.com/documents/RP-008353-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008351-DS)
- [机械制图，DXF](https://pip.raspberrypi.com/documents/RP-008352-DS)
...
### 树莓派1 型号 A+
- [原理图，修订版 1.1](https://pip.raspberrypi.com/documents/RP-008350-DS)

### 树莓派Zero
- [原理图，修订版 1.3](https://pip.raspberrypi.com/documents/RP-008366-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008365-DS)
- [案例图纸，PDF - 空白盖子](https://pip.raspberrypi.com/documents/RP-008362-DS)
- [案例图纸，PDF - GPIO 盖](https://pip.raspberrypi.com/documents/RP-008364-DS)
- [案例图纸，PDF - 相机盖](https://pip.raspberrypi.com/documents/RP-008363-DS)

### 树莓派Zero W
- [原理图，修订版 1.1](https://pip.raspberrypi.com/documents/RP-008368-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008367-DS)

### 树莓派Zero 2W
- [原理图](https://pip.raspberrypi.com/documents/RP-008360-DS)
- [机械图纸，PDF](https://pip.raspberrypi.com/documents/RP-008358-DS)
- [测试焊盘位置](https://pip.raspberrypi.com/documents/RP-008361-DS)
...
:::tip
上述温度、迟滞和转速数值可通过调整各种 `fan_tempN`、`fan_tempN_hyst` 和 `fan_tempN_speed` dtoverlay 设置进行调节（其中 `N` 取值为 0、1、2 或 3）。详见[覆盖层README文件](https://github.com/raspberrypi/linux/blob/rpi-6.12.y/arch/arm/boot/dts/overlays/README)获取完整说明。例如，在`/boot/firmware/config.txt`中添加`dtparam=fan_temp0=55000`，将使风扇保持关闭状态，直至树莓派5的温度达到55°C。
:::
...
### 重置 Raspberry Pi 引导加载程序 {#bootloader_reset}
本节提供通过重新刷写引导加载程序来重置它的说明。通常在系统无法启动时恢复系统，或重置引导加载程序配置时会这样做。

如果您不是要重置引导加载程序，而是想升级引导加载程序版本或选择不同的启动顺序，请参阅 **配置** 页面的 [更新引导加载程序](./configuration.mdx#update-bootloader-version)。

以下说明仅适用于在板载 SPI flash EEPROM 中存储引导加载程序的较新树莓派型号：

* 自树莓派 4B 起的旗舰单板计算机 (SBC)。
* 自 CM4 起的计算模块（包括 CM4S）。
* 所有键盘电脑（树莓派 400、500 和 500+）。

在其他树莓派型号上，引导 ROM 始终会加载位于启动介质引导分区中的 `bootcode.bin` 文件。

#### 诊断引导加载程序问题 {#bootloader_diagnostics}

如果您的树莓派设备无法启动，引导加载程序会通过以下方式提供反馈：

* **LED 活动。** 树莓派上的绿色 LED 可通过特定的闪烁模式指示特定错误。请查看 [LED 警告闪烁代码](./configuration.mdx#led警告闪烁代码) 以根据闪烁模式识别错误。例如，绿色 LED 短闪七次表示未找到内核镜像。
* **HDMI 诊断屏幕。** 在较新的设备上，引导加载程序可以在启动期间通过 HDMI 显示诊断屏幕。如果您的树莓派连接了显示器，该屏幕会提供引导加载程序版本和构建日期、检测到的启动顺序以及在操作系统加载失败时的具体错误代码等信息。更多信息请参阅 [启动诊断](#启动诊断)。

#### 重新刷写引导加载程序 {#bootloader_reflash}
如果您的系统无法启动或您想重置引导加载程序配置，请重新刷写引导加载程序。重新刷写引导加载程序会清除可能导致树莓派无法启动的本地引导加载程序配置。

:::note
您也可以通过 USB `rpiboot` 重新刷写引导加载程序。此方法通常用于批量配置计算模块。通过 USB 重新刷写引导加载程序也是 CM4 和 CM4S 唯一可用的选项。更多信息请参阅 [计算模块硬件](./compute-module.mdx#计算模块-eeprom-bootloader)。
:::

要在您的树莓派设备上重新刷写引导加载程序：

1. 将一个备用或空白的 microSD 存储设备连接到您的主机电脑。
2. 在主机电脑上找到并打开 Imager。
3. 在 **设备** 选项卡中，从列表中选择您的树莓派型号，然后选择 **下一步**。
4. 在 **操作系统** 选项卡中，滚动到列表底部并选择 **Misc utility images > Bootloader > SD Card Boot**，然后选择 **下一步**。
5. 在 **存储** 选项卡中，选择您的存储设备，然后选择 **下一步**。
6. 在 **写入** 选项卡中，选择 **写入**。
7. 选择 **我已知晓，擦除并写入**；等待写入完成，然后选择 **完成**。
8. 从主机电脑中弹出存储设备，然后插入您的树莓派。
9. 通过接通电源启动树莓派。
10. 当绿色活动 LED 以稳定模式闪烁且 HDMI 显示屏显示绿色屏幕后，断开电源关闭树莓派并弹出启动介质。

### 更新 bootloader 配置 {#更新-bootloader}
...
A/B EEPROM bootloader 更新可防止树莓派设备在更新中断后无法启动——即使设备在更新过程中断电，也能保护设备。

支持 A/B 的固件将 EEPROM 分为两个分区：A 和 B。在任何时候，其中一个分区是"已提交"的，是设备默认启动的分区。更新会写入另一个分区，在新映像完整写入并校验完毕之前，已提交的分区保持不变。

此功能仅适用于树莓派 5、树莓派 Compute Module 5 和树莓派 5 键盘电脑设备。

支持 A/B 的固件还支持 `tryboot` 回滚机制，可在提交前测试新分区。更多信息请参阅 [`rpi-eeprom-ab` README](https://github.com/raspberrypi/utils/tree/master/rpieepromab)。

有关 *Raspberry Pi OS* 的 A/B 更新信息，请参阅 [A/B 启动更新](../services/connect.mdx#a-b-启动更新)。

##### 启用 A/B bootloader 更新

注意：启用 A/B 固件更新后，直接写入 EEPROM 的工具（如 `flashrom`）将不再可用。

要使用 `raspi-config` 启用或禁用 A/B 固件更新：

. 在命令行中，运行以下命令将 Raspberry Pi 软件更新到最新版本：
+
```sh
sudo apt update && sudo apt full-upgrade
```
+
. 在命令行中，运行以下命令打开 `raspi-config`：
+
```sh
sudo raspi-config
```
+
. 导航到 **`6 Advanced Options > A14 AB Firmware`** 并选择 `Enable` 或 `Disable`。
. 选择 **`Finish`** 并在提示时重启。
. 在命令行中，运行以下命令执行 A/B 更新。
+
```sh
rpi-eeprom-update
```
...
| pieeprom.sig | 引导加载程序映像（`pieeprom.upd`/`pieeprom.bin`）的sha256校验和（及可选时间戳） |
| vl805.bin | VLI805 USB固件 EEPROM 映像 – 仅限树莓派4B Rev1.3及更早版本 |
| vl805.sig | vl805.bin的sha256校验和 |

- 如果bootloader更新映像名为 `pieeprom.upd`，那么更新完成后，`recovery.bin` 将重命名为 `recovery.000`，然后重新启动系统。由于 `recovery.bin` 已不存在，ROM会从SPI闪存中加载新更新的bootloader，操作系统会正常启动。
...
- `.sig`文件的第一行包含对应映像文件的十六进制sha256校验和，可选地在下一行跟随映像的时间戳（`ts: unixtime`）；未来可能添加更多字段。
...
:::note[注意]
USB 设备启动在 [计算模块系列](#compute-module系列)、[Zero系列](#zero系列)和 [Model A 旗舰系列变体](#旗舰级单板计算机sbc系列) 上可用。
:::
...
主机启动适用于 [Compute Module 系列（自 Compute Module 3 起）](#compute-module系列)、[Zero 系列（自 Zero 2 W 起）](#zero系列)、Raspberry Pi 2B（1.2 版）、Raspberry Pi 3B 和 [所有的旗舰系列（自 Raspberry Pi 3B+ 起）](#旗舰级单板计算机sbc系列 )。Raspberry Pi 3A+ 支持大容量存储器启动，但不支持网络启动。
:::
...
适用于 [自 Compute Module 3 起的 Compute Module 系列](#compute-module系列)、[自 Zero 2 W 起的 Zero 系列](#zero系列)和 [自 Raspberry Pi 2B（1.2 版）起的所有旗舰系列设备](#旗舰级单板计算机sbc系列)。
:::
...
:::tip
早期版本的 Raspberry Pi 4 可能需要 [bootloader 更新](#更新-bootloader)才能从 USB 启动。
:::
...
树莓派3B+和Zero 2 W支持开箱即用的USB大容量存储启动功能。
...
在树莓派 2B v1.2 和 v1.3、3A+、3B 以及计算模块 3 和 3+ 上，必须先启用 [USB 主机启动模式](#usb主机启动模式)。这将启用 USB 大容量存储启动和 [网络启动](#网络启动) 功能。
...
[操作步骤](./getting-started.mdx#将操作系统安装到启动介质上)与 SD 卡相同 - 向 USB 存储设备刷入操作系统。
...
如 BCM2835 [Arm外围设备数据表](https://pip.raspberrypi.com/documents/RP-008249-DS)第 102 页所述，GPIO 线路的默认上拉电阻为 50KΩ。建议使用 5KΩ 的上拉电阻来上拉 GPIO 线路：这将允许 GPIO 正常工作，但不会消耗过多的功率。

## NVMe SSD 启动

NVMe（非易失性内存高速接口）是一种通过PCIe总线访问外部存储的标准。您可通过 Compute Module 4 I/O板的PCIe插槽、Compute Module 5 I/O板的 M.2插槽，或使用M.2 HAT+/M.2 HAT+ Compact扩展板连接树莓派5来连接NVMe驱动器。经过额外配置后，您可从NVMe驱动器启动系统。
...
- 将 PCIe 转换为 M.2 标准的适配器。
  - 对于 Raspberry Pi 5，我们推荐使用 [M.2 HAT+ 或 M.2 HAT+ Compact](../accessories/m2-hat-plus.mdx)，它可将 Raspberry Pi 的 *PCIe FFC* 插槽转换为 M Key 接口。
...
## HTTP启动 {#http启动}
网络安装功能通过以太网使用 HTTP 引导树莓派进入嵌入式[树莓派Imager](./getting-started.mdx#使用-imager-安装)。
...
要使用 HTTP 启动，[更新](#更新-bootloader) 到 2022 年 3 月 10 日或以后发布的引导加载程序。HTTP 启动需要有线以太网连接。
...
- [Arm外围设备数据表](https://pip.raspberrypi.com/documents/RP-008249-DS)第102页定义了GPIO的默认值。如果启动时的值不等于默认上拉，则启用该启动模式。
...
#### BOOT_UART
如果为`1`则启用到主 UART（`/dev/serial0` 指向的 UART）的固件调试输出。

在 Raspberry Pi 5 上，默认情况下这是 `UART10`，通过调试 UART 连接器对外暴露。但在某些配置中，Raspberry Pi 5 也会在 GPIO 14 和 15 上暴露主 UART（`UART0`）。在 Raspberry Pi 4 上，主 UART 在 GPIO 14 和 15 上暴露。更多信息请参阅[主 UART 和辅助 UART](./configuration.mdx#主uart和辅助uart)。

将接收调试终端配置为115200bps、8数据位、无奇偶校验位、1停止位。
...
此设置不适用于树莓派 5 及更高版本、树莓派 Compute Module 5 及更高版本以及树莓派 500 及更高版本，因为 [专用电源按钮](#电源按钮) 始终可用于从 `HALT` 或 `STANDBY` 唤醒。
...
即使关闭处理器，树莓派 400 上的专用电源按钮也会工作。默认情况下启用此行为，但可以设置 `WAKE_ON_GPIO=2` 以使用外部 GPIO 电源按钮代替专用电源按钮。

在树莓派 5 及更高版本和树莓派 500 及更高版本上，这会将 PMIC 置于 `STANDBY`（待机）模式，关闭所有输出。无需设置 `WAKE_ON_GPIO`，按下 [专用电源按钮](#电源按钮) 即可启动设备。
...
`RPIBOOT` 通常通过选择 CM4 或 CM5 IO 板上的 `nRPIBOOT` 跳线隐式启用。不过，在自定义 `BOOT_ORDER` 中设置它可用于开发或部署场景，例如通过使用 `set_reboot_order`。如果使用，务必将其设置为 `BOOT_ORDER` 中的最后一个选项，因为此启动模式不支持超时或重试。
...
若设置为非零值（单位为秒），则在引导程序中启用硬件看门狗定时器。若操作系统未能在指定时间内启动，看门狗将重置系统。

Arm CPU启动后，引导程序看门狗将立即自动取消。它在引导程序移交操作系统后**不会**监控操作系统。

此功能适用于无人值守或远程系统，可确保从启动失败（例如操作系统无法加载）中恢复。

默认值：`0`（禁用）

#### `BOOT_WATCHDOG_PARTITION`

当引导程序看门狗触发时，此属性指定重置后启动的分区编号。这允许自动切换至恢复分区或备用分区。

若未设置，引导程序将重试默认分区（0）。

可结合 [表达式过滤器](./config-txt.mdx#表达式过滤器) 使用，在看门狗触发特定分区重启时应用不同设置或选择不同启动流程。

有关如何使用高分区号检测看门狗触发，请参阅 [PARTITION](./raspberry-pi.mdx#partition) 属性说明。
...
#### REBOOT_ON_FATAL_ERROR

如果在启动过程中遇到致命错误，引导加载程序将在活动 LED 上显示错误模式三次，然后执行硬复位。这可以缓解由电源、HAT 或主板温度引起的间歇性硬件问题。此设置仅适用于引导加载程序，操作系统启动后无效。

重置前显示三次错误模式可限制重启速率。如需更快重启，请使用 `BOOT_WATCHDOG_TIMEOUT`。

设置 `REBOOT_ON_FATAL_ERROR=0` 可禁用自动重启，改为无限期显示错误模式。
...
仅适用于树莓派 4B、5、500 及 500+。

启动前，引导程序会检测 SD 卡电源开关的过流信号。若 SD 卡损坏导致短路，将产生过流信号并引发发热现象。

若检测到过流状态，引导程序将切断SD卡电源并等待五秒后重新检测。此错误将显示在诊断屏幕、UART串口及活动指示灯（1长闪+2短闪）上。

通过在引导程序配置中设置`SD_OVERCURRENT_CHECK=0`，可将过流检测从错误模式切换为非阻塞警告模式。
...
#### HDMI_DELAY
跳过 HDMI 诊断显示，最多 N 秒（默认 5 秒），除非发生致命错误。默认行为旨在避免在正常的 SD/USB 启动过程中短暂出现引导加载程序诊断屏幕。
...
- 在 2022 年之前，SD 启动不启用自更新。在树莓派4上，ROM 已经可以从 SD 卡加载 recovery.bin。在 CM4 上，自更新和 recovery.bin 都不起作用，需要 USB 启动（参见 [CM4 引导加载程序文档](./compute-module.mdx#计算模块-eeprom-bootloader)）。
...
当 `HTTP_HOST` 设置为自定义服务器时，用于 HTTPS 网络安装和 HTTP 启动的 DER 编码 CA 证书的 SHA-256 哈希（十六进制字符串）。CA 证书本身通过 `rpi-eeprom-config --cacertder` 嵌入到 EEPROM 镜像中。

当 `HTTP_CACERT_HASH` 与 `HTTP_HOST` 和 `HTTP_PATH` 一起指定时，网络安装和 HTTP 启动将通过 443 端口使用 HTTPS，而非通过 80 端口使用明文 HTTP。

详见 HTTP 启动了解更多详情。

默认值：`""`（未设置）

#### IMAGER_REPO_URL {#IMAGER_REPO_URL}

树莓派 Imager 应用程序通过启动时下载的JSON文件进行配置。默认情况下，树莓派映像程序使用位于 https://downloads.raspberrypi.com/os_list_imagingutility_v4.json 的JSON文件。

在EEPROM配置中设置`IMAGER_REPO_URL`，可指定在使用嵌入式树莓派映像程序进行网络安装时使用的JSON文件位置。

您也可通过以下任一方式修改应用程序使用的 JSON 文件 URL，使其提供您自定义的镜像：

* 启动 Imager 时传递 `--repo` 参数，例如：`rpi-imager --repo https://downloads.raspberrypi.com/os_list_imagingutility_v4.json`
* 在树莓派映像程序界面中：
  * 进入 **应用选项** 对话框
  * 在 **内容存储库** 处选择 **编辑**
    * 若使用本地计算机的JSON文件，请选择 **使用自定义文件** 并浏览至文件位置
    * 若需使用URL托管的JSON文件，请选择 **使用自定义URL** 并输入URL。
  * 点击**应用并重启**。Imager将重置设备及操作系统选项，并返回 **设备** 选项卡。
...
在分区遍历过程中，不会处理 `autoboot.txt` 文件，以避免循环依赖。假定请求的启动分区已失败，系统正在尝试恢复。

默认值：`1`

#### BOOTVAR0

允许设置由 `config.txt` 使用的条件变量 `bootvar0`。详见 [bootvar0](./config-txt.mdx#bootvar0)。

#### PCIE_PROBE

仅限 Raspberry Pi 5 及之后旗舰型号。

设置为 `1` 时，引导加载程序在启动过程中枚举 PCIe x1 接口以发现 NVMe 设备。对于不支持 HAT+ 规范的自定义 PCIe 扩展设计，需要此设置。符合 HAT+ 规范的设备会被自动检测，不需要此设置。
...
在树莓派 4 1.3 版及更旧版本上，可配置/长时间断电要求启用 XHCI 控制器，因此实际上是先短时间断电，然后再长时间配置断电。可以通过将该参数设置为零来跳过较长的可配置断电时间。
...
SDRAM_BANKLOW 控制系统地址空间内 SDRAM Bank 的排列方式。Bank 数量可选择映射至地址的高位（MSB），或位于行地址位与列地址位之间。
此设置将显著影响SDRAM性能。

未设置时，引导程序将选择默认选项。当前树莓派 4 系列默认值为 3，树莓派 5 系列默认值为 1。
...
此设置还会促使引导程序为内核添加NUMA区域数量配置项（"numa=fake=\<n\>"）。
为获得最佳性能，建议保留默认设置。

:::tip
在树莓派5系列设备上，若内核不支持NUMA，性能将受到负面影响。
:::

手动设置SDRAM_BANKLOW=3可缓解此性能损失，但追求最高性能时请确保NUMA可用。
...
#### config.txt段
读取 `config.txt` 文件后，GPU 固件 `start4.elf` 会读取引导加载程序 EEPROM 配置，并检查是否存在名为 `[config.txt]` 的配置段。如果`[config.txt]`段存在，那么从该部分开始到文件结束的内容都会被添加到内存中，与从引导分区读取的 `config.txt` 文件内容相同。这可用于将设置自动应用到每个操作系统，例如 dtoverlays。
...
[在树莓派上使用 DPI 显示器](https://pip.raspberrypi.com/documents/RP-003471-WP)<br />
...
### 禁用其他GPIO外设
必须禁用使用与 GPIO 引脚冲突的所有其他外设覆盖。在 `config.txt` 中，小心地注释掉或反转任何启用 I2C 或 SPI 的 dtparams：
```
dtparam=i2c_arm=off
dtparam=spi=off
```
...
有关 GPIO 引脚[物理特性](#gpio)的更多信息。

:::note[注意]
在计算模块设备上，可以将 VDD IO 从标准的 3.3V 改为别的值。在这种情况下，V<sub>OL</sub> 和 V<sub>OH</sub> 将根据[GPIO](#gpio)表格中的情况发生变化。
:::
...
### 一次性可编程设置
:::info[白皮书]
[在树莓派单板计算机上使用一次性可编程内存](https://pip.raspberrypi.com/documents/RP-003611-WP-Using-the-One-time-programmable-memory-on-Raspberry-Pi-single-board-computers.pdf)<br />
...
Raspberry Pi 5 的电路板左侧有一个 FPC 连接器。该连接器为快速外设提供了 PCIe Gen 2.0 ×1 接口。

要连接 PCIe [HAT+设备](https://pip.raspberrypi.com/documents/RP-008281-DS)，请将其连接到树莓派。Raspberry Pi 会自动检测到该设备。要连接非 HAT+ 设备，请将其连接到 Raspberry Pi，然后[手动启用 PCIe](#启用-pcie)。

有关 PCIe FPC 连接器引脚分配以及创建第三方设备、配件和 HAT 所需的其他详细信息，请参阅 [Raspberry Pi 的 PCIe 连接器](https://pip.raspberrypi.com/documents/RP-008298-DS) 标准文档。应与 [Raspberry Pi HAT+ 规范](https://pip.raspberrypi.com/documents/RP-008281-DS) 一并阅读。
...
| 树莓派500+ | 5.0A | 1.6A | 800mA (LED灯关闭且固态硬盘处于非活动状态) |
...
下表描述了不同型号的树莓派在各种工作负载下消耗的电流（单位为安培）：
...
:::
:::info[白皮书]
[树莓派4、5和CM4上的额外 PMIC 功能](https://pip.raspberrypi.com/documents/RP-004340-WP-Extra-PMIC-features-on-Raspberry-Pi-4-and-Compute-Module-4.pdf)

Raspberry Pi 4、5 和 CM4 上使用了许多不同的 PMIC 器件。所有 PMIC 除了提供电压供应外，还提供额外的功能。本文档介绍了如何通过软件访问这些功能。
:::

#### 降低 Raspberry Pi 5 关机时的功耗

默认情况下，Raspberry Pi 5 关机时的功耗约为 1W 至 1.4W。可以通过使用 `sudo rpi-eeprom-config -e` 手动编辑 EEPROM 配置来降低功耗。编辑设置并添加
...
:::info[白皮书]
[打造弹性更强的文件系统](https://pip.raspberrypi.com/documents/RP-003610-WP)
...
## 实时时钟 (RTC)
树莓派5搭载了一个实时时钟（RTC）模块。该模块可通过板载J5（BAT）接口接入电池供电。当元件朝上且徽标方向正确时，J5接口位于电路板左近角处，紧邻USB-C电源接口右侧。

<img src="/img/computers/raspberry-pi-5/j5.jpg" alt="J5电池连接器" width="70%" />
*J5电池连接器*

您可以设置唤醒闹钟，该功能将使开发板进入超低功耗状态（约3毫安）。当达到闹钟时间时，开发板将重新启动。此功能适用于定时任务，例如延时摄影。
...
要以设定电压为电池充电，请在 `/boot/firmware/config.txt`"中添加 [`rtc_bbat_vchg`](https://github.com/raspberrypi/firmware/blob/master/boot/overlays/README#L400)：
...
要停止充电，请删除 `config.txt` 中包含 [`rtc_bbat_vchg`](https://github.com/raspberrypi/firmware/blob/master/boot/overlays/README#L400) 的所有行。
...
在树莓派4、400 和CM4上，有四条额外的 SPI 总线： SPI3 至 SPI6，每条总线有 2 个硬件芯片选择。这些额外的 SPI 总线可通过某些 GPIO 引脚上的备用功能分配使用。有关详细信息，请参阅 [BCM2711 Arm 外围设备](https://pip.raspberrypi.com/documents/RP-008248-DS)数据表。

[BCM2835 Arm 外围设备](https://pip.raspberrypi.com/documents/RP-008249-DS)数据表第 10 章介绍了主控制器。第 2.3 章介绍了辅助控制器。
...
SPI0 默认禁用。要启用它，请使用 [raspi-config](./configuration.mdx#交互式-raspi-config-文本用户界面-tui)，或确保 [/boot/firmware/config.txt](./config-txt.mdx#什么是configtxt) 中的 `dtparam=spi=on` 一行未被注释。默认情况下，它使用 2 条芯片选择线，但可以使用 `dtoverlay=spi0-1cs` 将其减少到 1 条。`dtoverlay=spi0-2cs` 也存在，在不带任何参数的情况下，它等同于 `dtparam=spi=on`。
...
```
:::note[注意]
所有树莓派计算机都会报告 BCM2835，即使是使用 BCM2836、BCM2837、BCM2711 和 BCM2712 处理器的计算机也不例外。您不应使用此字符串来检测处理器。请使用以下信息或 `cat /sys/firmware/devicetree/base/model` 解码修订码。根据您运行的内核版本不同，`cpuinfo` 输出中可能包含"Model"行，也可能不包含"Hardware"行。
:::
...
[^other]:对于内存大小为"其它"的情况，实际内存容量可通过 `rpi-sdram-size-gbit` [固件参数](./configuration.mdx#固件参数)确定。
...
| Raspberry Pi 2 Model B Rev 1.2| `raspberrypi`   | `2-model-b-rev2`   | `brcm`     | `bcm2837` |
...
| Raspberry Pi Compute Module 1 | `raspberrypi`   | `compute-module`   | `brcm`     | `bcm2835` |
