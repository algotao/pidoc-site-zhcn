...
sidebar_position: 6
draft: false
title_meta:  树莓派远程访问配置，设置SSH、VNC、NFS、Samba等
...
- [本站为国内用户提供的 PiDoc](#使用-pidoc-远程访问)
...
:::tip
若通过控制中心、`raspi-config` 或 `/etc/hostname` 修改树莓派系统主机名，Avahi 将自动更新 `.local` mDNS 地址。若忘记树莓派主机名，可在其他设备安装Avahi，通过[`avahi-browse`](https://linux.die.net/man/1/avahi-browse)浏览本地网络中的所有主机与服务。
:::
...
1. 按照 [Install with Imager](./getting-started.mdx#使用-imager-安装) 指南中的说明操作。
2. 在 **自定义>远程访问** 选项卡中，将 **启用SSH** 开关切换至激活状态。
3. 选择 **使用密码认证**，即可使用与物理操作树莓派时相同的用户名和密码登录。选择 **使用公钥认证**，则需[配置SSH密钥](./remote-access.mdx#手动配置-ssh-密钥)以实现基于SSH密钥的认证登录。
...
1. 参照[使用Imager安装](./getting-started.mdx#使用-imager-安装)指南配置启动映像。
2. 在 **远程访问** 选项卡中，将 *启用SSH* 开关切换至激活状态。.
3. 选择 *使用公钥认证*。
  * 若您已在`~/.ssh/id_rsa.pub`存储SSH公钥，Imager将自动填充该公钥至文本框。
  * 否则请浏览至公钥所在位置。.
4. 选择 **下一步** 继续镜像操作。
...
:::tip
本指南提供了生成新 RSA 密钥的说明。为提高安全性，你可以生成 [Ed25519](https://ed25519.cr.yp.to/) 密钥。向 `ssh-keygen` 传递 `-t ed25519` 并在引用公钥和私钥文件名时用 `ed25519` 替换 `rsa` 以使用 Ed25519 密钥。
:::
...
2. 选择桌面系统托盘中的 Raspberry Pi 图标。
3. 从菜单中选择 **Preferences** > **Control Centre**。
<img src="/img/computers/remote-access/raspberry-pi-configuration.png" alt="从系统托盘的首选项菜单中选择 控制中心" width="80%" />

4. 导航至 **Interfaces** 选项卡。
5. 选中 *VNC* 旁边的单选按钮，使其处于活动状态。
...
使用 [raspi-config](./configuration.mdx#交互式-raspi-config-文本用户界面-tui) 在命令行上启用 VNC 服务器。
...
7. 点击 "OK" 按钮验证 VNC 服务器。如果凭据正确无误，TigerVNC 将打开一个窗口，其中包含与你在 Raspberry Pi 上的账户相对应的桌面。您应该能够使用鼠标和键盘与桌面进行交互并输入文本。
...
更多信息，请参阅 [Connect 文档](../services/connect.mdx)。

## 使用 Pidoc 远程访问

`PiDoc` 是由本站提供的树莓派的一站式信息工具，其提供了设备指标监控。还可以通过本站公网堡垒机中转 ssh 的方式随时访问树莓派。适用于持续运行树莓派的用户或是使用树莓派的开发者。如您有一台或多台树莓派，通过PiDoc工具可很好的随时掌握其运行状态。

服务器位于国内，提供更稳定的连接，更高的传输带宽。欢迎免费使用。

更多信息，请参阅 [随时掌握你的树莓派状态](../pidoc/install_monitor.mdx)。
...
当 SMB 协议版本不匹配时，Linux Samba 客户端会返回误导性错误信息。默认情况下，Raspberry Pi OS 使用 2.1 及以上版本，与 Windows 7 及更高版本兼容。包括某些 NAS 设备在内的旧设备可能需要 1.0 版本。要修复此错误，请在挂载命令中添加一个版本条目（例如 `,vers=1.0`）：

```sh
sudo mount.cifs //IP/share /mnt/point -o user=<uname>,vers=1.0
```
您可能需要尝试不同的版本来匹配服务器版本。可能的值有：
| 版本 | 描述 |
| -- | -- |
...
sudo raspi-config
```
在 `raspi-config` 中选择 "`高级选项`"，然后选择 "`启动顺序`"，再选择 "`网络启动`"。然后必须重启设备，才能将启动顺序的更改写入引导加载程序 EEPROM。重新启动树莓派后，检查启动顺序是否已变为 `0xf21`：
```sh
