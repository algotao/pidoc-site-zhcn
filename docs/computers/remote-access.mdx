...
sidebar_position: 7
draft: false
title_meta:  树莓派远程访问配置，设置SSH、VNC、NFS、Samba等(官网24年12月)
description: 树莓派远程访问。通过ssh远程访问，配置免密ssh登录。使用scp安全复制文件。使用rsync同步文件。使用nfs及samba等服务提供网上文件共享。使用VNC服务远程访问桌面。使用apache设立web服务。
keywords: [树莓派, Raspberry Pi, 远程控制, 远程访问, VNC, NFS, SSH, Apache, 网络启动]
last_update: 
  date: 2024-12-21
---
