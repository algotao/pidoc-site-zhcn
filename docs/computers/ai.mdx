...
sidebar_position: 8
draft: false
title_meta: 使用树莓派 AI 人工智能神经网络加速器运行摄像头软件(官网25年09月)
description: 快速在树莓派5上部署AI加速方案！本指南详细介绍如何安装Hailo NPU（如Hailo-8L），通过Raspberry Pi AI Kit或AI HAT+硬件与64位Raspberry Pi OS Bookworm系统，搭配rpicam-apps演示套件（支持YOLOv5/6/8、YolovX等模型），实现摄像头实时对象检测、图像分割、姿势估计等AI功能。提供硬件连接、驱动安装、依赖配置及性能优化（如启用PCIe Gen 3.0）全流程步骤，并附模型资源与社区支持链接，助您轻松开启树莓派人工智能应用开发。
keywords: [树莓派5 AI, Hailo NPU, Raspberry Pi AI Kit, AI HAT+, rpicam-apps, 对象检测, 图像分割, 姿势估计, YOLO模型, 神经网络加速, 树莓派AI开发, Hailo-8L, PCIe Gen 3.0, 摄像头演示]
last_update: 
  date: 2025-12-20
---
...
* 安装 64 位 Raspberry Pi OS _Bookworm_
