...
sidebar_position: 4
draft: false
title_meta: 树莓派硬件功能配置文件(config.txt)全功能详解(官网26年02月)
...
### 安全启动属性配置
:::info[如何使用树莓派安全启动]
[如何使用树莓派安全启动](https://pip.raspberrypi.com/documents/RP-003466-WP-Boot-Security-Howto.pdf)

本白皮书介绍如何在基于树莓派4的设备上实现安全启动。有关安全启动实施方法的概述，请参阅[树莓派4启动安全白皮书](https://pip.raspberrypi.com/documents/RP-004651-WP-Raspberry-Pi-4-Boot-Security.pdf)。安全启动系统适用于基于 buildroot 的操作系统映像；不建议或不支持将其用于树莓派OS。
:::
...
### 监控核心温度
:::info[白皮书]
[冷却树莓派设备](https://pip.raspberrypi.com/documents/RP-003608-WP-Cooling-a-Raspberry-Pi-device.pdf)
...
* "启动变量"包括`boot_arg1`、`boot_count`、`boot_partition`和`partition`。
* 启动变量始终为小写。
* 整数常量可以以十进制或十六进制形式书写。
