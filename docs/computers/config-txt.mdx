...
sidebar_position: 4
draft: false
title_meta: 树莓派硬件功能配置文件(config.txt)全功能详解
description: 深入了解树莓派config.txt配置文件，它是树莓派启动过程中的关键组件，负责在GPU读取后初始化Arm CPU和Linux系统。本文详细介绍了config.txt的文件格式、高级功能、常用选项、属性配置、GPIO控制、超频选项以及监控和维护树莓派设备的方法，是树莓派用户优化和定制系统设置的必备指南。
keywords: [树莓派, config.txt, GPU, Arm CPU, Linux, 启动过程, 文件格式, 高级功能, 常用选项, 属性配置, GPIO控制, 超频选项, 监控, 维护]
last_update: 
  date: 2026-08-02
---
...
某些旧版 `config.txt` 选项已不再获得官方支持。这些选项列于 [旧版 config.txt 选项](./legacy-config-txt.mdx) 中，本文未作涵盖。
...
默认情况下，当连接到 4K 显示器时，某些型号会选择 30Hz 刷新率。使用此选项可选择 60Hz 刷新率。受此设置影响的型号仅在 HDMI0 上支持 4Kp60 输出。启用此设置会增加功耗和温度。
...
作为一种特殊情况，如果调用时没有任何值 - `dtoverlay=` - 则该选项标志着覆盖参数列表的结束。如果在任何其他 `dtoverlay` 或 `dtparam` 设置之前使用，则会阻止加载任何 HAT 覆盖层。

更多详情，请参阅 [DTB、覆盖和 config.txt](./configuration.mdx#dtb覆盖层和configtxt)。

#### dtparam

Raspberry Pi 设备的设备树配置文件支持各种参数，例如启用 I2C 和 SPI 接口。许多 DT 覆盖层都可以通过使用参数进行配置。这两种类型的参数都可以使用 `dtparam` 设置来提供。此外，覆盖参数还可以附加到 `dtoverlay` 选项中，用逗号分隔，但要注意行长限制为 98 个字符。
...
`start_file` 指定要使用的 VideoCore 固件文件。`fixup_file` 指定用于修复 `start_file` 中使用的内存位置的文件，使其与 GPU 内存分割相匹配。

`start_file` 和 `fixup_file` 是一对匹配的文件 – 使用不匹配的文件将导致电路板无法启动。这是一个高级选项，因此我们建议您使用 `start_x` 和 `start_debug`，而不是此选项。

:::note[注意]
不能以这种方式选择降频固件（`start*cd.elf` 和 `fixup*cd.dat`）– 否则系统将无法启动。启用降频固件的唯一方法是指定 `gpu_mem=16`。降频固件删除了对编解码器、3D和调试日志的支持，并将初始启动帧缓存限制为 1080p @16bpp – 尽管 KMS 可以在稍后阶段用最多 32bpp 的 4K 帧缓存替换它，就像使用任何固件一样。
:::

:::note[注意]
Raspberry Pi 5、500、500+，和Compute Module 5 固件自带引导加载程序 EEPROM。
:::
...
#### sha256
若设置为非零值，则启用对已加载文件（内核、初始化ramdisk、设备树 .dtb 文件及覆盖层）的SHA256哈希值记录功能，该哈希值由`sha256sum`工具生成。启用后日志输出将发送到UART，也可通过`sudo vclog --msg`查看。此选项在调试启动问题时可能有用，但会导致启动时间增加**数秒**。所有平台默认值为0。

#### uart_2ndstage
如果 `uart_2ndstage` 为 `1`，则启用 UART 的调试记录。该选项也会自动启用 `start.elf` 中的 UART 日志记录。启动选项页面对此也有说明。

`BOOT_UART` 属性也可启用引导加载程序 UART 日志，但不会在 `start.elf` 中启用 UART 日志，除非同时设置了 `uart_2ndstage=1`。

在 Raspberry Pi 5 上，`BOOT_UART` 使用专用调试排针。
...
将 `boot_arg1` 的值通过一个重置安全寄存器传递给引导加载程序，以便在重启后使用。
参见 [`boot_arg1`](#boot_arg1) 以获取更多详细信息。

要在运行时设置重启参数而无需编辑 `config.txt`，可以使用 `vcmailbox`。例如，要在下次重启时将 `boot_arg1` 设置为 `42`：
...
设置通过重置安全寄存器在重启后传递给引导加载程序的 [BOOT_ORDER](./raspberry-pi.mdx#boot_order) 的值。与 `tryboot` 类似，这是一个一次性设置，并在使用后自动清除。

此属性可用于调试不同的 [BOOT_ORDER](./raspberry-pi.mdx#boot_order) 设置。或者，它可用于具有电源控制和 `nRPIBOOT` GPIO 的配置系统，以在不指定 EEPROM 配置中的 [条件筛选器](#条件筛选器) 语句的情况下覆盖启动模式。

要在运行时设置重启顺序而无需编辑 `config.txt`，可以使用 `vcmailbox`。例如，要在下次重启时测试网络启动：

```sh
sudo vcmailbox 0x0003808b 4 4 0xf4612
sudo reboot
```
...
若设置为非零值（单位为秒），此属性将启用硬件看门狗定时器，并在系统启动时移交至操作系统（OS）。若操作系统未能定期"重置"或复位看门狗，系统将在指定超时时间后自动重启。

该属性设置`systemd`的`watchdog.open_timeout`参数，用于控制操作系统初始化并开始服务看门狗的时间限制。该值通过内核命令行传递给操作系统。为确保持续运行，操作系统还需定期重置看门狗，通常由`systemd`中的`RuntimeWatchdogSec`参数控制。更多信息请参阅[systemd看门狗文档](https://www.freedesktop.org/software/systemd/man/systemd-system.conf.html#RuntimeWatchdogSec=)。

:::tip
在树莓派操作系统*Bookworm*版本中，`RuntimeWatchdogSec`参数默认**未启用**，必须先在`/etc/systemd/system.conf`中配置此设置，才能使用固件内核看门狗功能。

若同时设置了 `BOOT_WATCHDOG_TIMEOUT`（EEPROM/引导程序设置，仅支持树莓派4和5）和 `kernel_watchdog_timeout`，引导程序将在操作系统启动时无缝切换至内核看门狗。这可实现从上电到操作系统运行期间的连续看门狗保护。

建议优先使用`kernel_watchdog_timeout`而非`dtparam=watchdog`，因为前者会显式设置`open_timeout`参数，确保看门狗在systemd接管前始终处于活动状态。
:::

这有助于确保系统在启动过程完成后能够从操作系统卡死或崩溃中恢复。

默认值：`0`（禁用）

#### `kernel_watchdog_partition`

若内核看门狗触发（具体来说，即操作系统未能在超时内重置看门狗），此属性将指定重置后启动的分区编号。此功能可实现自动故障转移至恢复分区或备用分区。

可结合 [表达式过滤器](./config-txt.mdx#表达式过滤器) 使用，在看门狗触发特定分区重启时应用不同设置或选择不同启动流程。

有关如何使用高分区号检测看门狗触发机制的详细信息，请参阅[PARTITION](./raspberry-pi.mdx#partition)属性。
...
#### bootloader_update
可以将此选项设置为 0 来阻止自更新，而无需更新 EEPROM 配置。这在通过网络启动更新多台树莓派时非常有用，因为此选项可以按每台树莓派进行控制（例如，通过 `config.txt` 中的序列号过滤器）。

默认值：`1`

### 安全启动属性配置
:::info[如何使用树莓派安全启动]
[如何使用树莓派安全启动](https://pip.raspberrypi.com/documents/RP-003466-WP)

本白皮书介绍如何在基于树莓派4的设备上实现安全启动。有关安全启动实施方法的概述，请参阅[树莓派4启动安全白皮书](https://pip.raspberrypi.com/documents/RP-004651-WP)。安全启动系统适用于基于 buildroot 的操作系统映像；不建议或不支持将其用于树莓派OS。
:::
...
#### program_pubkey
如果该属性设置为 `1`，那么 `recovery.bin` 将把 EEPROM 映像中公钥的哈希值写入 OTP。设置后，引导加载程序会拒绝使用不同 RSA 密钥签名的 EEPROM 映像或未签名的映像。
...
如果此属性设置为 `1`，则 `recovery.bin` 将向 OTP 写入一个值，以防止 ROM 加载不支持 `secure-boot` 的旧版本第二阶段引导加载程序。这可防止通过回滚到旧版本引导加载程序来关闭 `secure-boot`。因此，如果生产设备启用了 `secure-boot`，则必须设置此属性。

此属性在 `recovery.bin` `2025/05/16` 及更新版本中，如果 `program_pubkey=1`，则会自动设置。

默认值：`0`

#### program_rpiboot_gpio

仅适用于 Raspberry Pi 4B 和 Raspberry Pi 400。

Compute Module 4 和 4S 配备专用的 `nRPIBOOT` 跳线，用于选择 `RPIBOOT` 模式。Raspberry Pi 4B 和 Raspberry Pi 400 没有专用的 `nRPIBOOT` 跳线，因此必须选择以下其中一个 GPIO 引脚作为 `nRPIBOOT` 使用。
...
GPIO 可以在操作系统启动后用作通用输入输出引脚。然而，您应确认此 GPIO 配置不会与任何可能在启动过程中将 GPIO 拉低的 HAT 发生冲突。

尽管 `secure-boot` 要求在 Raspberry Pi 4B 和 Raspberry Pi 400 上设置此属性，但它并不依赖于 `secure-boot`。例如，`RPIBOOT` 可用于自动化测试。

出于安全考虑，此 OTP 值只能通过 `RPIBOOT` 进行编程。因此，您必须首先使用 `erase_eeprom` 清除引导加载程序 EEPROM。空白的 EEPROM 会导致 ROM 切换到 `RPIBOOT` 模式，从而允许设置此选项。
...
设置 `enable_jtag_gpio=1` 可为 GPIO 引脚 22-27 选择 Alt4 模式，并建立某些内部 SoC 连接，从而为 Arm CPU 启用 JTAG 接口。此设置适用于所有型号的树莓派。
...
## 超频选项
内核有一个 [CPUFreq](https://www.kernel.org/doc/html/latest/admin-guide/pm/cpufreq.html) 驱动程序，默认情况下已启用省电调速器，安装 [raspi-config](./configuration.mdx#交互式-raspi-config-文本用户界面-tui) 后在启动过程中切换为按需调速器。使用按需调速器后，CPU 频率将随处理器负载而变化。您可以使用 `*_min` 配置选项调整最小值，或通过应用静态缩放调速器（powersave 或 performance）或 `force_turbo=1` 来禁用动态时钟。
...
:::warning[警告]
将任何超频参数设置为 [`raspi-config`](./configuration.mdx#overclock) 所用值以外的值，可能会在 SoC 内设置一个永久位。这样就有可能检测到你的 Raspberry Pi 曾经超频。当 `force_turbo` 设置为 `1`，并且任何 `over_voltage_*` 选项设置为 `0` 以上时，超频位就会设置。更多信息，请参阅 [关于 Turbo 模式的博文](https://www.raspberrypi.com/news/introducing-turbo-mode-up-to-50-more-performance-for-free/)。
:::
...
| 选项 | Pi Zero W | Pi 1 | Pi 2 | Pi 3 | Pi 3A+<br />Pi 3B+ | CM4 & Pi 4B<br />⇐ R1.3 | Pi 4B R1.4 | Pi 400 | Pi Zero 2W | Pi 5/500/500+ |
| -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | 
...
| v3d_freq | 300 | 250 | 250 | 400 | 400 | 500 | 500 | 500 | 300 | 960 |
...
### 监控核心温度
:::info[白皮书]
[冷却树莓派设备](https://pip.raspberrypi.com/documents/RP-003608-WP)
...
| `[pi500]` | 500/500+。同时查看 `[pi5]` 内容） |
| `[cm0]` | CM0。同时查看 `[pi02]` 内容） |
| `[cm1]` | CM1。同时查看 `[pi1]` 内容） |
| `[cm3]` | CM3。同时查看 `[pi3]` 内容） |
...
```ini
# 引导加载程序 EEPROM 配置。
# 如果 PM_RSTS 为分区 62，则设置引导加载程序属性以禁用
# 标清高速并显示 HDMI 诊断
# 使用调试选项从分区 2 启动。
[partition=62]
# 只有高分区（>31）可以重新映射。
...
表达式过滤器支持使用一组简单的运算符将无符号整数"启动变量"与常量进行比较。它旨在支持OTA更新机制、调试和测试。

* "启动变量"包括`boot_arg1`、 `cust_otpN`, `bootvar0`、`boot_count`、`boot_partition`和`partition`。
* 启动变量始终为小写。
* 整数常量可以以十进制或十六进制形式书写。
* 表达式条件过滤器没有副作用，例如没有赋值运算符。
* 与其他过滤器类型一样，表达式过滤器不能嵌套。
* 使用 `[all]` 过滤器重置表达式和其他所有条件过滤器类型。

语法：
```ini
# ARG 是启动变量
# VALUE 和 MASK 是无符号整数常量
[ARG=VALUE]      # 当 (ARG == VALUE) 时选中
[ARG&MASK]       # 当 ((ARG & VALUE) != 0) 时选中
[ARG&MASK=VALUE] # 当 ((ARG & MASK) == VALUE) 时选中
[ARG<VALUE]      # 选择条件为 (ARG < VALUE)
[ARG>VALUE]      # 选择条件为 (ARG > VALUE)
```


#### boot_arg1
仅适用于 Raspberry Pi 5 及更新设备。

`boot_arg1` 变量是一个 32 位用户自定义值，存储在重置安全寄存器中，可实现参数在重启后传递。

通过 `config.txt` 将 `boot_arg1` 设置为 42：

```ini
set_reboot_arg1=42
```

`set_reboot_arg1` 属性用于设置下次启动时的值。它不会更改当前值，当前值仍由配置解析器识别。

通过 vcmailbox 将 `boot_arg1` 设置为 42：
```sh
sudo vcmailbox 0x0003808c 8 8 1 42
```

通过 vcmailbox 读取 `boot_arg1`：
...
操作系统启动时 `boot_arg1` 变量的值可通过 [设备树](./configuration.mdx#设备树) 在 `/proc/device-tree/chosen/bootloader/arg1` 读取

#### `cust_otpN`

`cust_otpN` 启动变量（其中 `N` 的取值范围为 0-7）对应于八个客户 OTP 行。
这使您能够根据存储在客户 OTP 中的持久化值选择不同的配置。

以下 `config.txt` 代码片段仅在客户 OTP 第 0 行的第 0 位被设置时更改 `cmdline` 文件：
...
有关编程客户 OTP 行的更多信息，请参阅 [写入和读取客户 OTP 值](./raspberry-pi.mdx#写入和读取客户otp值)。

#### bootvar0

仅适用于树莓派4及更新设备。

`bootvar0`变量是一个32位用户自定义值，通过`rpi-eeprom-config`设置后，可在`config.txt`中作为条件变量使用。

例如，通过 `rpi-eeprom-config` 将 `bootvar0` 设为 42：
```ini
BOOTVAR0=42
```

在 `config.txt` 中条件使用 `bootvar0`：
...
这使得通用镜像（即使用相同`config.txt`文件）能够根据持久化的`rpi-eeprom-config`设置支持不同配置。

#### boot_count
仅适用于 Raspberry Pi 5 及更新设备。

`boot_count` 变量是一个存储在复位安全寄存器中的 8 位值，在启动时递增（达到 256 后回绕到 0）。如果断开电源，该值将被清零。

通过 vcmailbox 读取 `boot_count`：
```sh
...
通过 vcmailbox 设置/清除 `boot_count`：
```sh
# 将 boot_count 设置为零以清除其值。
sudo vcmailbox 0x0003808d 4 4 0
```

操作系统启动时 `boot_count` 的值可通过 [设备树](./configuration.mdx#设备树) 在 `/proc/device-tree/chosen/bootloader/count` 读取

#### boot_partition

`boot_partition`变量可用于根据`config.txt`文件从哪个分区加载（在处理[autoboot.txt](#autoboottxt)后），选择加载替代操作系统文件（如`cmdline.txt`）。此功能旨在与 `A/B` 启动系统配合使用，其中 `autoboot.txt` 文件用于实现 `A` 和 `B` 镜像在启动分区中安装相同文件的需求。

`boot_partition` 的值可能与请求的 `partition` 变量不同，如果在 [autoboot.txt](#autoboottxt)，或者如果指定的分区不可引导且 [PARTITION_WALK](./raspberry-pi.mdx#partition_walk) 在 EEPROM 配置中启用。

示例 `config.txt` - 选择与 `A/B` 引导文件系统匹配的根文件系统：
```ini
# 使用不同的命令行文件指向不同的根文件系统，具体取决于系统从哪个分区启动。
[boot_partition=1]
cmdline=cmdline_rootfs_a.txt  # 指向根文件系统 A

[boot_partition=2]
cmdline=cmdline_rootfs_b.txt  # 指向根文件系统 B
```

`boot_partition` 的值（即用于启动操作系统的分区）可从 [设备树](./configuration.mdx#设备树) 中的 `/proc/device-tree/chosen/bootloader/partition` 读取。

#### partition
`partition` 变量可用于根据请求的分区号（`sudo reboot N`）选择替代启动流程，或通过直接使用`PM_RSTS`看门狗寄存器进行操作。
...
在启动开始时，`tryboot` 的值可以通过 [设备树](./configuration.mdx#设备树) 在 `/proc/device-tree/chosen/bootloader/tryboot` 读取。
...
要控制 HDMI 设置，请按照 **配置** 页面中 [显示设置](./configuration.mdx#显示设置) 部分的说明进行操作。
...
将 `awb_auto_is_greyworld` 设置为 `1`，可让内部不支持 greyworld 选项的库或应用程序使用 NoIR 摄像头捕捉有效的图像和视频。它将自动 awb 模式切换为使用灰色世界算法。只有在无红外摄像头或高质量摄像头的[红外滤光片](../accessories/camera.mdx#红外滤光片)被移除时，才需要这样做。
