...
sidebar_position: 5
draft: false
title_meta: 从源码构建自定义树莓派Linux内核
description: 深入了解树莓派Linux内核的构建、配置与贡献流程。本文详细介绍了内核的托管、升级、本地与交叉编译方法，以及如何通过menuconfig进行配置和打补丁。此外，还阐述了如何安装内核头文件、贡献代码至Raspberry Pi内核和上游Linux内核，助力开发者深入定制和优化树莓派系统。
keywords: [树莓派, Linux内核, 内核构建, 交叉编译, 内核配置, menuconfig, 内核补丁, 内核头文件, 内核贡献, Raspberry Pi内核, 上游Linux内核]
last_update: 
  date: 2025-09-27
---
...
通常的 Raspberry Pi OS [更新进程](./os.mdx#update-software)会自动将内核更新到最新的稳定版本。如果你想尝试最新的不稳定测试内核，可以 [手动更新](./os.mdx#upgrade)。
