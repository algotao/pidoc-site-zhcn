...
sidebar_position: 15
draft: false
title_meta: 树莓派旧版系统config.txt配置文件详解
description: 深入了解树莓派的旧版config.txt配置选项，这些选项在树莓派OS Bookworm中已不再使用，但对旧版操作系统用户和裸机开发者仍具参考价值。本文详细介绍了启动选项、GPIO控制、超频设置、内存与视频配置、HDMI模式、旧版杂项选项等，帮助用户在特定场景下进行系统优化和故障排除。
keywords: [树莓派, config.txt, 旧版配置, 启动选项, GPIO控制, 超频, 内存配置, 视频选项, HDMI模式, 旧版杂项选项, 树莓派OS Bookworm]
last_update: 
  date: 2026-05-03
---
...
### HDMI模式
:::note[注意]
对于具有多个 HDMI 端口的设备，某些 HDMI 命令可以应用于任何端口。您可以使用语法`<command>:<port>`（其中端口为 0 或 1）来指定设置应应用于哪个端口。如果在不需要端口号的命令中指定端口号，端口将被忽略。有关语法和替代机制的更多详情，请参阅文档 [条件部分](#旧版的条件过滤器)中的 HDMI 子部分。
...
将 `disable_overscan` 设置为 `1`，以禁用固件设定的默认超扫描值。
...
在使用旧版（pre-KMS）图形驱动程序的多显示器系统中，该选项会强制将特定的内部显示设备作为第一个 Linux 帧缓冲器（即 `/dev/fb0`）。
...
#### display_lcd_rotate
对于旧版图形驱动程序（树莓派4之前型号的默认设置），使用 `display_lcd_rotate` 旋转或翻转 LCD 方向。参数与 `display_hdmi_rotate` 相同。另请参阅 `lcd_rotate`。
...
:::important[重要]
使用 VC4 KMS 图形驱动程序时，整个显示管道由 Linux 管理，其中包括 HDMI 输出。这些设置仅适用于旧版的 FKMS 和基于固件的图形驱动程序。
:::
