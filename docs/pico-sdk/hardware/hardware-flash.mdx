...
`void flash_do_cmd_cs (const uint8_t *txbuf, uint8_t *rxbuf, size_t count, uint cs)`<br/>&emsp;执行双向 QSPI 命令。

`static void flash_do_cmd (const uint8_t *txbuf, uint8_t *rxbuf, size_t count)`<br/>&emsp;在片选 0 上执行双向 flash 命令。
...
底层函数，用于在连接到 QSPI 接口的设备上执行串行命令。字节从 txbuf 同时发送，并从 rxbuf 接收。因此，两个缓冲区的长度必须相同，即 count（整个事务的长度）。这对于读取芯片的元数据（如设备 ID 或 SFDP 参数）非常有用。
...
在 RP2040 上，片选索引会被忽略，因为只有一个片选。

**参数**

- `txbuf`: 指向将发送的字节缓冲区的指针
- `rxbuf`: 指向接收数据将写入的字节缓冲区的指针。txbuf 和 rxbuf 可以是同一个缓冲区。
