...
有关 RP 系列微控制器中时钟的更多详情，请参阅相应的数据手册。
...
- `bool clock_configure_mhz (clock_handle_t clock, uint32_t src, uint32_t auxsrc, uint32_t src_freq_mhz, uint32_t freq_mhz)`: 以 1MHz 精度配置指定时钟。
...
- `static float frequency_count_mhz (uint src)`: 使用频率计数器测量时钟频率。
...
此函数与 clock_configure 的不同之处在于它不会以那么高的精度配置时钟，因此不需要引入 64 位除法函数，从而在应用程序未以其他方式使用 64 位除法时减小代码体积。
...
- src: 要测量的时钟源，参见数据手册中的 FC0_SRC 寄存器
