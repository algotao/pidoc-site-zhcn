...
PIO 的完整详细信息请参阅相应的 RP 系列微控制器数据手册。请注意，RP2350 PIO 实现中有额外功能，因此在编写需要同时在 RP2040 和 RP2350 上运行的 PIO 代码时需要注意。
...
- typedef struct pio_program pio_program_t: PIO 程序定义。
...
- `static void pio_sm_restart (PIO pio, uint sm)`: 以已知内部状态重启状态机。
- `static void pio_restart_sm_mask (PIO pio, uint32_t mask)`: 以已知内部状态重启多个状态机。
...
- `void pio_set_input_sync_bypass_with_mask (PIO pio, uint32_t bypass_enables, uint32_t pin_mask)`: 为 PIO 实例的多个引脚启用/禁用输入同步器旁路。
- `void pio_set_input_sync_bypass_with_mask64 (PIO pio, uint64_t bypass_enables, uint64_t pin_mask)`: 为 PIO 实例的多个引脚启用/禁用输入同步器旁路。
...
此结构体描述一个 PIO 程序：编码指令数组以及将其加载到 PIO 实例所需的元数据。它是 `pioasm` 工具（以 `<program_name>_program` 形式）输出的类型，并由 `pio_add_program`、`pio_can_add_program` 及相关函数使用。也可以在使用 `pio_instructions.h` 中的辅助函数在运行时汇编程序时手动填充。
...
- program: 要添加的 PIO 程序
- pio_out: 返回 PIO 硬件实例，若无可用 PIO 则返回 NULL
- sm_out: 返回已占用的 PIO 状态机的索引
- offset_out: 返回程序起始处的指令内存偏移
...
`pio_claim_free_sm_and_add_program` 的此变体在 RP2350B 上很有用，在该型号中，每个 PIO 实例必须设置"GPIO Base"以寻址 32 个 GPIO（0\-&gt;31）或 32 个 GPIO（16-47）。单个 PIO 实例无法同时与引脚 0\-&gt;15 或 32\-&gt;47 交互。

此方法接受关于所需 GPIO 引脚的额外信息（通过 gpio_start 和 gpio_count），并在必要时可选地设置未使用 PIO 实例的 GPIO 基址（参见 `pio_set_gpio_base`）。
...
- sm_out: 返回已占用的 PIO 状态机的索引
- offset_out: 返回程序起始处的指令内存偏移
- gpio_start: 所需的最低 GPIO 编号（RP2350B 上为 0-47，否则为 0-31）
- gpio_count: 所需的连续 GPIO 数量
- set_gpio_base: 若在具有正确 GPIO 基址的 PIO 实例上没有空闲 SM，但有未使用的 PIO 实例，则该 PIO 将被重新配置以使此方法成功。注意，当 PICO_PIO_USE_GPIO_BASE=0 时此参数被忽略；即默认情况下在非 RP2350B 上被忽略
...
对 mask 中选中的每个状态机执行与 pio_sm_restart 相同的操作。参见 pio_sm_restart 以了解被清除的内部状态的完整列表以及不受影响的内容（值得注意的是，启用/运行状态和程序计数器被保留）。

**参数**

- pio: PIO 实例；例如 [pio0] 或 pio1
- mask: 要重启的状态机索引位掩码（位 0 = 状态机 0，以此类推）
...
为 PIO 实例的多个引脚启用/禁用输入同步器旁路。

**参数**

- pio: PIO 实例；例如 [pio0] 或 pio1
- bypass_enables: 要设置的值——1 = 启用旁路，0 = 禁用旁路（若 pin_mask 中对应位已设置）
- pin_mask: 每个引脚一位，表示该引脚对应的旁路使能位是否应被更新

#### pio_set_input_sync_bypass_with_mask64
...
清除所选状态机上的以下内容：输入和输出移位计数器、输入移位寄存器的内容、延迟计数器、等待 IRQ 状态、任何写入 SMx_INSTR 或由 OUT/MOV EXEC 运行的停滞指令，以及因 OUT_STICKY 而保持断言的任何引脚写入。

不受影响：状态机的启用/运行状态（参见 pio_sm_set_enabled）、其程序计数器、其输出移位寄存器的内容以及其 X 和 Y 暂存寄存器。要重启时钟分频器，请使用 pio_sm_clkdiv_restart；要将程序计数器设置为特定值，请在此调用后通过 pio_sm_exec 执行 JMP。
...
此方法反复重新配置目标状态机的引脚配置，并执行 'set' 指令在所有引脚上设置引脚方向，然后将状态机的引脚配置恢复到之前的状态。
...
- pin_mask: 每个引脚一位，表示该引脚的对应 pin_dir 是否应被应用。
...
此方法反复重新配置目标状态机的引脚配置，并执行 'set' 指令在所有引脚上设置值，然后将状态机的引脚配置恢复到之前的状态。
...
:::::note
当 PICO_PIO_USE_GPIO_BASE == 1 时，pio_sm_set_config 会忽略未调用相应 sm_config_ 引脚函数的字段，因此您不必将未使用引脚集的设置移动到正确的引脚范围。因此，始终通过相应的 sm_config_ 函数显式配置从引脚 0 开始的引脚范围（例如 sm_config_set_out_pin_base(config, 0)）是最佳实践，因为在 RP2350B 上 pio_get_default_sm_config 的引脚范围默认值现在是 GPIO_BASE + 0 而非 0。
:::::
...
| --- | --- |
| Clock Divider | 1 |
| Out Pins | 0 starting at 0 (见下文注释) |
| Set Pins | 0 starting at 0 (见下文注释) |
| In Pins | 32 starting at 0 (见下文注释) |
| Side Set Pins (base) | 0 (见下文注释) |
| Side Set | disabled |
...
:::::note
在启用了 PICO_PIO_USE_GPIO_BASE 的 RP2350B 上，Out/Set/In/Side Set 引脚基址实际上相对于应用 pio_sm_config 的 PIO 实例的 GPIO_BASE 为 0，因此任何未显式指定的引脚基址都不会包含在选择兼容 PIO 实例的逻辑中。
:::::

因此，例如，如果您打算在 RP2350B 上使用从引脚 0 开始的 Out 引脚，应显式调用 sm_config_set_out_pin_base(config, 0) 或 sm_config_set_out_pins(config, 0, count)。
