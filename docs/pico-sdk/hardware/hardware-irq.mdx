...
- `uint32_t irq_get_mask (void)`: 获取执行核心上当前启用的掩码。
- `uint32_t irq_get_mask_n (uint n)`: 获取执行核心上当前启用的掩码。
...
| PWM_IRQ_WRAP | 选择 PWM 的 WRAP IRQ 输出。 |
| USBCTRL_IRQ | 选择 USBCTRL 的 IRQ 输出。 |
...
| IO_IRQ_BANK0 | 选择 IO_BANK0 的 IRQ 输出。 |
| IO_IRQ_QSPI | 选择 IO_QSPI 的 IRQ 输出。 |
| SIO_IRQ_PROC0 | 选择 SIO 的 PROC0 IRQ 输出。 |
| SIO_IRQ_PROC1 | 选择 SIO 的 PROC1 IRQ 输出。 |
| CLOCKS_IRQ | 选择 CLOCKS 的 IRQ 输出。 |
| SPI0_IRQ | 选择 SPI0 的 IRQ 输出。 |
| SPI1_IRQ | 选择 SPI1 的 IRQ 输出。 |
| UART0_IRQ | 选择 UART0 的 IRQ 输出。 |
...
mask - 32 位掩码，其中置 1 的位对应已启用的中断 [中断编号]
