...
- `uint32_t powman_timer_get_lposc_calib_freq (void)`: 获取低功耗振荡器的校准频率。
- `void powman_timer_set_1khz_tick_source_xosc (void)`: 使用晶体振荡器作为 powman 定时器源。
- `void powman_timer_set_1khz_tick_source_xosc_with_hz (uint32_t xosc_freq_hz)`: 使用晶体振荡器（指定频率）作为 powman 定时器源。
...
- `static void powman_timer_pause (void)`: 暂停 powman 定时器。
...
注意，如果正在关闭切换核心，则需要在此函数返回后调用 __wfi()，否则转换不会发生。
...
注意，此函数会先禁用再重新启用闹钟。此函数仅控制闹钟本身，若要使用闹钟唤醒 powman，应使用 powman_enable_alarm_wakeup_at_ms。
...
返回的频率是存储在 OTP 中的值，若未设置 OTP 值、OTP 值超出范围或 PICO_POWMAN_CALIBRATE_LPOSC_FROM_OTP 为 0，则返回 0。

:::::note
这是一个弱函数，因此可以被覆盖以提供自定义校准方法（例如使用 frequency_count_khz）
:::::
...
:::::note
频率设置为 powman_timer_get_lposc_calib_freq 返回的值
:::::
...
- lposc_freq_hz: 指定精确的 lposc 频率以进行校准，或为 0 以使用现有值
...
:::::note
下次启动时，定时器将从上次设置的时间继续，因此如果您想暂停定时器并从当前时间恢复，应使用 powman_timer_pause
:::::
