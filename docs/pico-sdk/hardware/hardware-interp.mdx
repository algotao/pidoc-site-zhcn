...
`static void interp_set_base (interp_hw_t *interp, uint index, uint32_t val)`<br/>&emsp;按索引设置插值器基址寄存器。

`static uint32_t interp_get_base (interp_hw_t *interp, uint index)`<br/>&emsp;按索引获取插值器基址寄存器的内容。
...
返回指定通道的原始移位和掩码值，不添加 BASE0/BASE1。
