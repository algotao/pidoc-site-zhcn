...
- `static void reset_block_num (reset_num_t block_num)`: 复位指定的 HW 块。
- `static void unreset_block_num (reset_num_t block_num)`: 将指定的 HW 块移出复位状态
- `static void unreset_block_num_wait_blocking (reset_num_t block_num)`: 将指定的 HW 块移出复位状态并等待完成。
- `static void reset_unreset_block_num_wait_blocking (reset_num_t block_num)`: 复位指定的 HW 块，然后将其移出复位状态并等待完成。
