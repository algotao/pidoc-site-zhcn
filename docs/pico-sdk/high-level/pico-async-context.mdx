...
`async_context_poll` - 此上下文不是线程安全的，用户负责定期调用 async_context_poll()（参见 async_context_poll），并在没有其他事情要做时使用 async_context_wait_for_work_until() 在两次调用之间休眠，直到需要工作时再唤醒。
...
- typedef struct async_context_poll async_context_poll_t: 用于简单单核轮询循环的 async_context 类型。
...
用于简单单核轮询循环的 async_context 类型。

保存 async_context_poll 实例的状态，该实例通过轮询而非使用中断或线程来处理异步工作。
