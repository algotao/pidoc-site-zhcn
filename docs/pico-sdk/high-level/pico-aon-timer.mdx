...
* `#define [AON_TIMER_IRQ_NUM]()` <span className="chip-tag rp2350">RP2350</span>
...
`absolute_time_t aon_timer_get_absolute_time (void)`<br/>&emsp;以绝对时间形式获取 AON 定时器的当前时间。

`static absolute_time_t aon_timer_make_timeout_time_ms (uint32_t ms)`<br/>&emsp;获取距 AON 定时器当前时间指定毫秒数后时间戳的便捷方法。
...
#### AON_TIMER_IRQ_NUM <span className="chip-tag rp2350">RP2350</span>

#define AON_TIMER_IRQ_NUM()

返回 AON 定时器抽象底层硬件中断的 irq_num_t。
