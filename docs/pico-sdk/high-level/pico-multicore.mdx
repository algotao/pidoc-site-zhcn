...
| 位 | 描述 |
| --- | --- |
| 3 | 粘性标志，表示 RX FIFO 在空时被读取（ROE）。此次读取已被 FIFO 忽略。 |
| 2 | 粘性标志，表示 TX FIFO 在满时被写入（WOF）。此次写入已被 FIFO 忽略。 |
| 1 | 若此核心的 TX FIFO 未满（即 FIFO_WR 准备好接收更多数据），值为 1。 |
| 0 | 若此核心的 RX FIFO 非空（即 FIFO_RD 有效），值为 1。 |
...
`static bool multicore_doorbell_is_set_current_core (uint doorbell_num)` <span className="chip-tag rp2350">RP2350</span><br/>&emsp;判断指定门铃是否在当前核心上处于激活状态。

`static bool multicore_doorbell_is_set_other_core (uint doorbell_num)` <span className="chip-tag rp2350">RP2350</span><br/>&emsp;判断指定门铃是否在另一个核心上处于激活状态。
...
默认情况下，为了方便起见，如果 core 1 尚未通过 multicore_launch_core1 函数启动，或已通过 multicore_reset_core1 重置，multicore_lockout_start_ 函数将在 core 0 上成功执行。因此，在 core 0 处于 multicore_lockout_ 函数内部时调用 multicore_launch_core1 是不安全的（尽管你不太可能这样做）。可以通过设置 PICO_MULTICORE_LOCKOUT_BEFORE_CORE1_STARTED=0 来禁用此默认行为，在这种情况下，core 1 必须正在运行且处于 "victim 已初始化" 状态，才能在 core 0 上调用 multicore_lockout_start 函数。
...
- `bool multicore_lockout_ready (void)`: 判断从此核心调用 multicore_lockout_start 函数是否安全。
...
判断从此核心调用 multicore_lockout_start 函数是否安全。

**返回**

如果可以从此核心安全调用 multicore_lockout_start_blocking() 和 multicore_lockout_start_timeout_us() 则返回 true

:::::note
当 PICO_MULTICORE_LOCKOUT_BEFORE_CORE1_STARTED=1 时，如果 core 1 尚未通过 multicore_launch_core1 函数启动，或已通过 multicore_reset_core1 重置，则在 core 0 上调用此函数返回 true。否则，它返回与 multicore_lockout_victim_is_initialized(other_core) 相同的值。此行为旨在方便可能想在 core 0 上执行操作、但可能尚未启动 core 1 的应用程序。
:::::
...
此状态在核心随后被重置后仍会保留（通过 pico_multicore_reset_core1 重置的除外）；因此，建议在重置之前已初始化过的核心后，始终重新调用 multicore_lockout_victim_init()。
