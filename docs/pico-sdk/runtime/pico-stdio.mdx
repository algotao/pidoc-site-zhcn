...
`int stdio_put_string (const char *s, int len, bool newline, bool cr_translation)`<br/>&emsp;将缓冲区打印到 stdout，可选择追加换行符。
...
将缓冲区打印到 stdout，可选择追加换行符。

将 s 写入每个已注册的 stdio 驱动程序。调用在每个驱动程序接受字节后返回。

**参数**

- s: 要打印的字符
- len: s 的长度，或 -1 表示用 strlen 计算
- newline: 若为 true 则在字符串后追加换行符
- cr_translation: 若为 true 则执行换行符到回车符的转换

**返回**

从 s 写入的字符数（不包括任何追加的换行符）；若并发 stdout 写入正在进行且设置了 PICO_STDIO_IGNORE_NESTED_STDOUT 则返回 0。
