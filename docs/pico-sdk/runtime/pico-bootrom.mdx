...
REBOOT2_FLAG_REBOOT_TYPE_NORMAL - 重启进入正常启动路径。

REBOOT2_FLAG_REBOOT_TYPE_BOOTSEL - 重启进入 BOOTSEL 模式。p0 - 一组标志：0x01：DISABLE_MSD_INTERFACE - 禁用 BOOTSEL USB 驱动器；0x02：DISABLE_PICOBOOT_INTERFACE - 禁用 PICOBOOT 接口；0x10：GPIO_PIN_ACTIVE_LOW - p1 中指定的 GPIO 低电平有效（也必须设置 GPIO_PIN_SPECIFIED）；0x20：GPIO_PIN_SPECIFIED - 在 p1 指定的 GPIO 上启用活动指示灯。p1 - 用作活动指示灯的 GPIO 编号（由 p0 中的 GPIO_PIN_SPECIFIED 标志启用）。

REBOOT2_FLAG_REBOOT_TYPE_RAM_IMAGE - 重启进入 RAM 中的镜像。搜索 RAM 或 XIP RAM 区域中的可运行镜像并执行它。这是拖动 RAM UF2 到 BOOTSEL USB 驱动器时使用的重启类型。p0 - 区域起始地址（字对齐）。p1 - 区域大小（字对齐）。

REBOOT2_FLAG_REBOOT_TYPE_FLASH_UPDATE - REBOOT2_FLAG_REBOOT_TYPE_NORMAL 的变体，用于 flash 更新后使用。这是拖动 flash UF2 到 BOOTSEL USB 驱动器后使用的重启类型。p0 - 已更新 flash 区域的起始地址。如果此地址与分区或插槽的起始地址匹配，则在启动时（有选择时）优先处理该分区或插槽。这种启动类型便于 TBYB 和版本降级。

REBOOT2_FLAG_REBOOT_TYPE_PC_SP - 重启到特定的 PC 和 SP。注意：ARM-NS 变体不允许此操作。p0 - 开始执行的初始程序计数器（PC），Arm 必须设置最低位，RISC-V 必须清除最低位。p1 - 初始堆栈指针（SP）。

所有上述内容都可以通过 OR 加入可选标志：

REBOOT2_FLAG_REBOOT_TO_ARM - 将两个核心切换到 Arm 架构（而不是保持原样）。如果不支持 Arm 架构，调用将以 BOOTROM_ERROR_INVALID_STATE 失败。REBOOT2_FLAG_REBOOT_TO_RISCV - 将两个核心切换到 RISC-V 架构（而不是保持原样）。如果不支持 RISC-V 架构，调用将以 BOOTROM_ERROR_INVALID_STATE 失败。REBOOT2_FLAG_NO_RETURN_ON_SUCCESS - 看门狗硬件是异步的。设置此位强制此方法在重启成功启动时不返回。
...
:::::note
在 RP2350A-A2 芯片上，勘误 RP2350-E3 阻止了活动 LED 在 Arm 下工作。默认定义 PICO_BOOTROM_WORKAROUND_RP2350_A2_ACTIVITY_LED_BUG=1，使此方法重启到 RISC-V USB 启动以正确显示活动 LED。
:::::
...
- usb_activity_gpio_pin_active_low: 活动 GPIO 低电平有效（在 RP2040 上忽略）。RP2350 A4 芯片 bootrom 中的 bug 意味着此参数在该版本的 RP2350 上无效。
