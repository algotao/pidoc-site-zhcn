...
* `#define __check_type_compatible(type_a, type_b) _Static_assert(__builtin_types_compatible_p (type_a, type_b), __STRING(type_a) " is not compatible with " __STRING(type_b));`
...
- `static uint8_t rp2350_rom_version (void)` <span className="chip-tag rp2350">RP2350</span>: 返回 RP2350 ROM 版本号。
...
节属性为 `.time_critical.<group>`，用于保持与旧链接器脚本的兼容性。
...
用于在倒数第二个 SRAM bank（称为"scratch X"）中放置数据的节属性宏。
...
用于在最后一个 SRAM bank（称为"scratch Y"）中放置数据的节属性宏。
...
```c
uint32_t __in_scratch_y("my_group_name") foo = 23;
```
...
PSRAM 通常用于额外的数据节。可以将数据放置在已初始化或未初始化的 PSRAM 中，具体取决于数据如何加载到 PSRAM 中。

例如，一个放置在 PSRAM 中的 `uint32_t` 变量：
...
节属性为 `.psram_initialised.<group>` 或 `.psram_uninitialised.<group>`
...
用于在未初始化 PSRAM 中放置的节属性宏。

用于未初始化数据的 `__in_psram` 版本。
...
用于在 XIP SRAM 中放置的节属性宏。

XIP 缓存可用作额外数据节的 SRAM，但如果您的二进制文件从 Flash 运行（例如默认二进制类型），则会产生性能损失。

例如，一个放置在 XIP SRAM 中的 `uint32_t` 变量：
...
用于在默认 POWMAN 复位后保持的节中放置的节属性宏。

以此方式标记的数据将在默认 POWMAN 复位后保留其值，并在任何其他复位时被清零。

例如，一个 `uint32_t` foo，初始时被清零，然后在程序因默认 POWMAN 复位重启后保留其值。

```c
uint32_t __persistent_data(foo);
```

节属性为 `.persistent_data.<name>`
...
默认情况下，此宏与 `__in_ram` 相同，但可以通过 `PICO_NOT_IN_FLASH_PLACEMENT` 定义进行调整。此定义可使用 `pico_set_not_in_flash_placement` CMake 函数设置。

例如，对于仅使用 core 0 的二进制文件，可以选择使用 `pico_set_not_in_flash_placement(TARGET scratch_x)` 将此代码/数据放置在 scratch X 中，以将其移出条带化 SRAM。
...
修饰函数名，使该函数从 RAM 执行（假设编译器未将其内联到 flash 函数中）以避免可能的 flash 延迟。
...
函数使用 `__not_in_flash` 放置，默认为 `__in_ram`。
...
修饰函数名，使该函数从 RAM 执行以避免可能的 flash 延迟。默认情况下，此宏的实现与 `__no_inline_not_in_flash_func` 相同，但语义不同，`__time_critical_func` 可以受到特殊处理以减少从 flash 函数调用此类函数的开销。
...
默认情况下，函数使用 `__in_ram` 放置，但可以通过 `PICO_TIME_CRITICAL_PLACEMENT` 定义进行调整。此定义可使用 `pico_set_time_critical_placement` CMake 函数设置。

例如，对于不从 flash 执行的二进制文件（例如 copy_to_ram 和 no_flash），可以选择使用 `pico_set_time_critical_placement(TARGET xip_ram)` 将这些函数放置在 XIP RAM 中，因为 XIP AHB 端口将 otherwise 闲置。
...
RP2350 芯片版本号（A2 为 2，A3/A4 为 3）。

#### rp2350_rom_version <span className="chip-tag rp2350">RP2350</span>
...
RP2350 ROM 版本号（RP2350-A2 为 2，RP2350-A3 为 3，RP2350-A4 为 4）。
