...
description=my-auth-key&ttl_days=1&device_name=pi5-001
```
...
```sh
curl --header 'Authorization: Bearer rporg_accesstokenhere' --data-urlencode 'description=my-auth-key' --data-urlencode 'ttl_days=1' --data-urlencode 'device_name=pi5-001' https://api.connect.raspberrypi.com/organisation/auth-keys
```
