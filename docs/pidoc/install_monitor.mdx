...
draft: false
title: 随时掌握你的设备状态
description: PiDOC远程指标监测工具，提供派的CPU负载、网络、磁盘、温度等监测的一站式信息。同时还能提供远程ssh功能，解决校园网等环境不能设置静态IP的问题。
...
`PiDoc` 是由本站提供的树莓派（也[兼容](#兼容性)其他派以及龙芯信创电脑）的一站式信息工具，其提供了设备指标监控。还可以通过本站公网堡垒机中转 ssh 的方式随时访问树莓派。适用于持续运行树莓派的用户或是使用树莓派的开发者。如您有一台或多台树莓派，通过PiDoc工具可很好的随时掌握其运行状态。

请到 [Pidoc远程管理网站](https://ops.pidoc.cn/) 注册一个账号，并按照指引接入新设备。
...
将以下命令复制到设备的bash窗口执行即可：
```sh
curl -fsSL https://dl.pidoc.cn/pidoc/install.sh | bash
```
...
```
Pidoc Endpoint v2.0.12
```

### 安装特定版本

可以指定版本号安装。如当前设备已安装PiDoc，则指定版本不能小于已安装版本。例如安装 v2.0.11 版本：

```sh
PIDOC_VERSION=2.0.11 bash <(curl -sSL https://dl.pidoc.cn/pidoc/install.sh)
```

各种版本及架构可在 [这里](https://dl.pidoc.cn/pidoc/) 查看。


## 功能
* **实时监控**：`CPU负载`、`网络`、`磁盘`、`温度` 等监测
* **网页终端**：在世界任何地方安全访问您的设备
* **堡垒SSH**：提供双层防护，以SSH Proxy 的方式提供堡垒SSH
* **本地转发**：将远程设备的端口映射到本地，实现 `VNC连接`、`我的世界` 游戏连接等
...
PiDOC 内置免费 AI 对话能力，在设备安装 v2.0.12+ 版本的客户端后（[安装方法](#在设备安装-pidoc)），设备会自动在本地启动一个 **OpenAI 兼容 API**，为 **PicoClaw**、**OpenClaw** 等应用提供 AI 能力。

本地 API 地址：`http://127.0.0.1:11434/v1`

### PicoClaw / OpenClaw 配置

在应用的 LLM 设置中选择 OpenAI 兼容 API，填入以下信息：
...
所有请求通过 SSH 隧道转发到 PiDOC 代理服务器，代理端调用云端大模型为您服务。
...
- 模型查询：`GET /v1/models` （返回模型 ID: `auto`）
- 流式与非流式响应
- 工具调用（Function Calling） — AI 编码助手（如 CodeBuddy / Cline）也可使用

### 使用须知

- **免费**：该功能完全免费，无需额外付费
- **Token 随意填写**：API Key 字段填写任意字符串即可，不验证
- **纯文本模型**：仅支持文字对话，不支持图片等多模态输入
- **有限流保护**：每分钟 10 次请求 / 50000 tokens，每小时 100 次 / 500000 tokens，每天 1000 次 / 2000000 tokens，超限后返回错误

:::note
本地 API 在 PiDOC 客户端与服务器连接成功后自动启动，断开连接后自动停止。请确保客户端运行正常后再使用。
:::

:::warning
该功能为公共资源，请合理使用。频繁触发限流说明您的客户端可能存在自动重试或过度调用的情况，请调整使用频率。
:::

## 注册

去 [Pidoc远程管理网站](https://ops.pidoc.cn/)  注册一个账号。
...
登录 [Pidoc远程管理网站](https://ops.pidoc.cn/) ，在 **设备管理** 页面点击 **增加** 按钮。
<img src="/img/pidoc/add_dev.png" width="80%" alt="接入新设备" />

复制命令行并在安装好Pidoc程序的设备上执行。
...
为了解决设备在内网环境中无法控制的问题，`PiDoc`工具开发了 `远程访问` 功能。在页面左侧导航栏中点击 `远程访问` 会打开一个页面新窗口。
...
点击Tab **+** 号或页面中间的 **新建连接** ，然后选择 **SFTP文件**。

请根据您设备的实际情况选择填写 **用户名**、**端口**，选择 **密码认证** 或 **私钥认证**。

<img src="/img/pidoc/web_sftp.png" width="80%" alt="网页SFTP" />

连接成功后将看到服务器的文件列表。你可以切换目录、支持文件多选、支持文件拖放上传、支持文件下载。

<img src="/img/pidoc/web_sftp_1.png" width="80%" alt="网页SFTP" />

### VNC桌面

点击Tab **+** 号或页面中间的 **新建连接** ，然后选择 **VNC桌面**。

请根据您设备的实际情况选择填写 **端口**，如VNC端口可访问，则会弹出窗口提供输入 **用户名与密码**。如无法访问，请检查设备上的 VNC 服务是否已开启以及端口是否正确

<img src="/img/pidoc/web_vnc.png" width="80%" alt="网页VNC" />

## 堡垒SSH

如果您想使用 SSH 的 scp 传输文件，或是想通过 VSCode等等 IDE远程访问设备，可以使用 `堡垒SSH` 功能。

堡垒SSH 本质上是一个 SSH 代理，它会将您的请求转发到设备上。对于该功能的使用需要您有一定的SSH配置知识。
...
堡垒机基本信息。地址 `ops.pidoc.cn`，端口 `64422`，用户名任意。
...
Host rpi-5
    # 设备SSH用户名，请根据您的实际情况修改
    User pi
    # 设备SSH端口，请根据您的实际情况修改
    Port 22
    # 设备的私钥文件路径
    IdentityFile ~/.ssh/id_rsa
    # 指定代理跳转的主机配置项，这里我们使用 `ops.pidoc.cn`
    ProxyJump ops.pidoc.cn
...
The authenticity of host '[ops.pidoc.cn]:64422 ([101.43.71.125]:64422)' can't be established.
...
如果您有多台设备，则可使用 SSH Config 通配机制，这样是无须为每一台设备指定登录信息，除非该设备需要特殊配置。
```
...
本地转发功能可以将远程设备的端口映射到本地，实现 VNC 连接、我的世界 游戏连接等。该功能需要PidocDesktop 客户端支持。

下载：[PidocDesktop Win64 V0.1.2](https://dl.pidoc.cn/pidoc/pidocdesktop-0.1.2-windows-amd64.zip)

:::warning
PidocDesktop 在Beta阶段，如遇问题请入群反馈。
:::

### 客户端登录

登录 PidocDesktop 客户端需要在 [Pidoc远程管理网站](https://ops.pidoc.cn/) 中生成 Agent Token。然后将 Token 填写到 Pidoc Desktop 客户端的登录信息中。
...
在 Windows 中打开 Pidoc Desktop 客户端，填入 Token 登录。

<img src="/img/pidoc/agent_login.png" width="30%" alt="登录 PidocDesktop，填入 Token" />
<p />

登录成功后将显示设备列表。
<img src="/img/pidoc/agent_dev_list.png" width="30%" alt="PidocDesktop，填入显示设备列表" />
...
设备的 VNC 端口默认为 5900，所以这里我们填入 5900。
...
:::tip
现在就可以通过 VNC 客户端连接到您的设备了，地址为 `localhost:5900`。
:::
...
设备的 SSH 端口默认为 22，所以这里我们目标端口填入 22。
...
:::tip
现在就可以通过 SSH 客户端连接到设备了，地址为 `localhost:22`。
:::

## 网站转发

如果您有OpenClaw、Home Assistant等需要通过网站访问的应用，则可以使用网站转发功能。

例如您访问 OpenClaw 的 WebUI，则可以通过网站转发功能将 OpenClaw 的 WebUI 转发到本地。

:::important[重要]
网站转发功能登录ops.pidoc.cn，在登录有效期内才可正常访问。
:::

<img src="/img/pidoc/web_forward.png" width="50%" alt="网站转发" />

## 配置文件

PiDoc 配置文件位于 `/etc/pidoc/config.json`，该文件在执行 `pidoc service install` 时自动生成。
...
| server_addr | string | 堡垒机服务器地址。当前仅有 `ops.pidoc.cn:3322` 可用，可不填写。未来会提供更多服务器地址以分担压力。 | 2.0.7 |
| websocket | boolean | 是否启用 WebSocket。在一些网络环境中，需要启用 WebSocket 穿越防火墙。 | 2.0.6 |
| websocket_url | string | WebSocket 地址穿越服务URL。当前仅有 `wss://ops.pidoc.cn/ws` 可用，可不填写。未来会提供更多服务器地址以分担压力。 | 2.0.7 |
| forward_port_whitelist | array of int | 端口转发白名单，用于限制端口转发的范围以提升安全性。 | 2.0.7 |
| client_id | string | 自定义客户端ID（可选，为空则自动获取设备序列号）。同一账号下的多个设备的客户端ID不能相同 | 2.0.8 |

```json
{
  "private_key": "5woUnsa2BGl3CGWXy20ovB2dUXG95gKz2wZKBu0kdds=",
  "server_addr": "ops.pidoc.cn:3322",
  "websocket": true,
  "websocket_url": "wss://ops.pidoc.cn/api/endpoint/ws-tunnel",
...
如果失败可以考虑检查：
1. 设备 SSH 服务未启动或端口不对
2. 登录 https://ops.pidoc.cn 检查：设备是否显示在线（绿色状态）？
...
如果还是卡住，检查设备上的 pidoc 客户端（堡垒机代理）是否正常运行：
```sh
# 在设备上执行
...
支持的设操作系统包括但不限于：Raspberry Pi OS、Debian、Ubuntu、Armbian、Kali Linux、Linux Mint、银河麒麟 Kylin、统信 UOS、深度 Deepin、麒麟信安 OS 等。

支持的芯片架构：arm64、armhf、amd64、i386、riscv64、loong64 等。
...
3. 用户SSH终端 - 您用来访问设备的工具(SSH客户端、浏览器WebSSH 或 Pidoc Desktop)
