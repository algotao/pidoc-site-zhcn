...
`PiDoc` 是由本站提供的树莓派（也[兼容](#兼容性)其他派）的一站式信息工具，其提供了设备指标监控。还可以通过本站公网堡垒机中转 ssh 的方式随时访问树莓派。适用于持续运行树莓派的用户或是使用树莓派的开发者。如您有一台或多台树莓派，通过PiDoc工具可很好的随时掌握其运行状态。
...
可以指定版本号安装。如当前设备已安装PiDoc，则指定版本不能小于已安装版本。例如安装 v2.0.6 版本：

```sh
PIDOC_VERSION=2.0.6 bash <(curl -sSL https://dl.pidoc.cn/pidoc/install.sh)
```
...
为了解决树莓派在内网环境中无法控制的问题，`PiDoc`工具开发了 `网页SFTP` 功能。在页面左侧导航栏中点击 `SSH连接` 会打开一个网页SSH页面新窗口。
...
下载：[PidocAgent Win64 V0.1.0](https://dl.pidoc.cn/pidoc/pidocagent-0.1.0-windows-amd64.zip)
...
登录 Agent 需要在 [Pidoc远程管理网站](https://ops.pidoc.cn/) 中生成 Agent Token。然后将 Token 填写到 Pidoc Agent 客户端的登录信息中。
...
在某些网络环境中，访问 github.com 可能存在稳定性差、速度慢等问题。其中一个原因是163骨干网由境外回程极为拥堵，造成使用体验不佳。

`PiDoc 2.0.9` 版本开始支持 github 访问线路优化。本站租用了 CN2 GIA 商业线路，提升访问 github 回程的稳定性，实测速度约为稳定的 1.1MB/s。

编辑配置文件 `/etc/pidoc/config.json`，启用配置项

```json
{
  "private_key": "你设备的密钥，无须更改",
  "http_proxy_enabled": true,  // 启用 HTTP 代理
  "http_proxy_listen": "localhost:38090", // HTTP 代理监听地址
  "http_proxy_domains": ["*.github.com", "*.githubusercontent.com"]  // HTTP 代理域名白名单。该配置为本地逻辑分支，确定是否直连。实际白名单仍由后端控制。
}
```
...
使用如下命令测试是否能否能稳定访问 github
```sh
https_proxy=http://localhost:38090 curl https://github.com
```

如能看到输出HTML，则表示可以稳定访问 github。

配置 git 程序使用代理，且只对 github.com 和 githubusercontent.com 域名生效。

```sh
git config --global http.https://github.com.proxy http://localhost:38090
git config --global http.https://githubusercontent.com.proxy http://localhost:38090
```

:::warning
1. 未使用VPN等技术，您的访问源/目等元组信息仍置于监管规则之下。（它不是 “翻墙” 或 “科学上网”）
2. 如您所在运营商阻塞了目标地址，则使用该功能也无法访问目标地址。它只提升稳定性，不提升穿越能力。
3. 如您无须设置即可稳定访问 github，则不要启用该项功能。带宽为商业付费，有一定成本。
4. 请尽量不要设置为系统级的代理，避免在pidoc程序未运行时影响当前设备访问网络。
:::
...
| http_proxy_enabled | boolean | 是否启用 HTTP 代理 | 2.0.9 |
| http_proxy_listen | string | HTTP 代理监听地址（默认 localhost:38090） | 2.0.9 |
| http_proxy_domains | array of string | HTTP 代理允许的域名列表，白名单由服务端控制，本地只做为备份措施。不在列表中的域名由pidoc本地转发。 | 2.0.9 |
...
```json
{
  ...
  "http_proxy_listen": "localhost:38090",
  "http_proxy_domains": ["*.github.com", "*.githubusercontent.com"]
}
```
