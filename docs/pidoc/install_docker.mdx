...
draft: false
title: 在树莓派安装Docker
description: 快速在树莓派上部署Docker！本文提供详细指南，涵盖系统要求（Raspberry Pi OS Bookworm 12/Bullseye 11）、国内镜像源配置（推荐阿里云/腾讯云替代Docker官方源）、Docker Engine及相关工具（docker-ce、docker-compose-plugin等）的安装步骤、验证方法（通过hello-world镜像）、普通用户权限配置（免sudo操作），以及卸载方法和镜像拉取加速解决方案（国内镜像源配置）。助您高效搭建树莓派容器化开发环境！
