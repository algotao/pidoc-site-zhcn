...
draft: false
title: 在树莓派安装Docker
description: 快速在树莓派上部署Docker！本文提供详细指南，涵盖系统要求（Raspberry Pi OS Trixie 13/Bookworm 12/Bullseye 11）、APT源配置（DEB822 .sources 格式与旧 .list 格式）、国内镜像源配置（推荐阿里云/腾讯云替代Docker官方源）、Docker Engine及相关工具（docker-ce、docker-compose-plugin等）的安装步骤、验证方法（通过hello-world镜像）、普通用户权限配置（免sudo操作），以及卸载方法和镜像拉取加速解决方案（国内镜像源配置）。助您高效搭建树莓派容器化开发环境！
keywords: [树莓派Docker, Raspberry Pi Docker, Docker Engine安装, 树莓派操作系统, Docker国内镜像源, Docker加速, Docker CE, docker-compose-plugin, 容器化部署, 树莓派容器, 免sudo Docker, 镜像拉取问题, DEB822, Trixie]
last_update: 
  date: 2026-06-24
---
...
:::note
Trixie 使用 **DEB822 格式** 的 `.sources` 文件，内容为多行键值对结构。
:::
...
sudo tee /etc/apt/sources.list.d/docker.sources > /dev/null <<EOF
Types: deb
URIs: http://mirrors.tencent.com/docker-ce/linux/debian
Suites: trixie
Components: stable
Architectures: $(dpkg --print-architecture)
Signed-By: /etc/apt/keyrings/docker.asc
...
<TabItem value="bookworm" label="Bookworm / Bullseye (12 / 11)">

:::note
Bookworm 和 Bullseye 使用传统的 **one-line 格式** 的 `.list` 文件。
:::
