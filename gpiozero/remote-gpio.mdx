...
draft: false
title: 4. 配置远程GPIO
description: 本文介绍GPIO Zero配置远程GPIO的方法。GPIO Zero支持多种引脚库，默认是RPi.GPIO，也可选用pigpio实现网络远程控制GPIO引脚，能通过另一台树莓派或PC控制连接在树莓派上的设备。详细说明了准备树莓派（不同系统安装pigpio、启用远程连接及启动守护进程）和控制计算机（不同系统安装相关库）的步骤，还介绍了通过环境变量和引脚工厂两种配置远程引脚的方式及使用示例。
keywords: [GPIO Zero, 远程GPIO, pigpio, RPi.GPIO, 引脚库, 树莓派, 控制计算机, 环境变量, 引脚工厂, 远程控制]
last_update: 
  date: 2024-06-13
---
...
GPIO Zero 支持多种不同的引脚实现（直接处理 GPIO 引脚的底层引脚库）。默认情况下会使用 [RPi.GPIO](https://pypi.python.org/pypi/RPi.GPIO) 库（假设系统中已安装），但也可选择指定一个库来使用。更多信息，请参阅 [API - 引脚文档页面](./api-pins)。
