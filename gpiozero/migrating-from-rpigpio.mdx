...
sidebar_position: 12
draft: false
title: 11. 从RPi.GPIO移植
description: 本文介绍从RPi.GPIO库向GPIO Zero库移植的相关内容。对比了两者在操作LED、按钮等设备上的不同方式，如RPi.GPIO需处理引脚编号和状态，GPIO Zero则通过设备类操作。还涵盖复合设备、PWM控制、引脚清理等方面，同时提及GPIO Zero提供更多功能，如对SPI设备支持、与GPIO引脚无关的设备类等，并解答了移植可能遇到的常见问题。
keywords: [GPIO Zero, RPi.GPIO, 移植, LED, 按钮, 复合设备, PWM控制, 引脚清理, 设备类, 常见问题]
last_update: 
  date: 2024-06-14
---
...
GPIO Zero 最初只是 RPi.GPIO 的顶层，但后来我们增加了对其他各种底层引脚库的支持。RPi.GPIO 目前是默认使用的引脚库。如需了解更多信息，请参阅 [更改引脚工厂](./api-pins)。
...
请查看支持的 [输出设备](./api-output.mdx) 的完整列表。其他输出设备也有类似的属性和方法名称。基类的命名有共性，如 `OutputDevice.is_active`，在设备类中有别名，如 `LED.is_lit`。
...
GPIO Zero 支持多个引脚库。默认情况下使用 `RPi.GPIO` 控制引脚，但也可以选择使用其他库，如支持网络控制 GPIO 的 `pigpio`。更多信息，请参阅 [API-引脚](./api-pins) 文档的"更改引脚工厂"部分和 [配置远程GPIO](./remote-gpio.mdx)。

使用 [API-引脚](./api-pins) 文档中介绍的Mock引脚可以在 PC 上运行 GPIO Zero，用于远程 GPIO 和测试目的。
